...
class List extends BlockFormat {
  ...
  static createListFromSelection(listType) {
    ...
    if (listType === 'checklist') {
      listItem.innerHTML = `<input type="checkbox"> ${selectedText}`;
    } else {
  ...
  static convertBlocksToList(blocks, listType) {
    ...
    blocks.forEach(block => {
      ...
      if (listType === 'checklist') {
        listItem.innerHTML = `<input type="checkbox"> ${block.textContent || ''}`;
      } else {
  ...
  static changeListType(existingList, newListType) {
    ...
    Array.from(existingList.children).forEach(item => {
      ...
      if (newListType === 'checklist') {
        const checkbox = item.querySelector('input[type="checkbox"]');
        const text = checkbox ? item.textContent.replace(/^[\s]*/, '') : item.textContent;
        newItem.innerHTML = `<input type="checkbox"${checkbox && checkbox.checked ? ' checked' : ''}> ${text}`;
      } else {
        // Remove checkbox if converting from checklist
