...
class TextAlignPicker {
  ...
  async createAlignmentButtons() {
    ...
    // Preload icons first
    const iconNames = this.options.alignments.map(align => align.icon);
    try {
      await IconLoader.preloadIcons(iconNames);
    } catch (error) {
      console.warn('Could not preload alignment icons:', error);
    }
    ...
    for (const alignment of this.options.alignments) {
      ...
      // Add icon
      try {
        let iconSvg = await IconLoader.getIcon(alignment.icon);
        alignButton.innerHTML = `
          <span class="icon-wrapper">${iconSvg}</span>
          <span class="label-text">${alignment.label}</span>
        `;
      } catch (error) {
        console.warn(`Could not load icon ${alignment.icon}:`, error);
  ...
  updateCurrentAlignment() {
    ...
    try {
      ...
      const blockElement = this.getBlockElement(range.commonAncestorContainer);
      
      if (blockElement) {
        const textAlign = window.getComputedStyle(blockElement).textAlign;
        this.currentAlignment = textAlign === 'left' || textAlign === 'start' || !textAlign ? 'left' : textAlign;
      } else {
        this.currentAlignment = 'left';
      }
