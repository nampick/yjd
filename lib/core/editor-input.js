...
// A file-chip href is safe when it is a normal navigable URL or an inert file
// data: embed; a rejected URL falls back to '#' (non-navigating) so the chip
// still renders but can't open a scriptable data document.
function safeChipHref(url) {
  return isSafeUrl(url || '', { allowDataFile: true }) ? String(url).trim() : '#';
}
...
export function applyEditorInput(Editor) {
  ...
  P.insertImageFile = function insertImageFile(file) {
    ...
    if (typeof cfg.upload !== 'function') {
      ...
      reader.onload = (ev) => {
        ...
        // makeImg (Image.create) scheme-checks src; the raw fallback (used only
        // when the image format isn't registered) must do the same so an
        // image/svg+xml data URL can't slip in unvalidated.
        const el = makeImg(src);
        const html = el ? el.outerHTML
          : (isSafeUrl(src, { allowDataImage: true })
              ? `<img src="${src}" class="inserted-image" style="max-width:100%" contenteditable="false">` : '');
        if (!html) return;
        this.focus();
  ...
  /** Read a video File and insert it as an inline <video> player (data URL, or via options.video.upload) at the caret. */
  P.insertVideoFile = function insertVideoFile(file) {
    if (!file || !file.type || !file.type.startsWith('video/')) return;

    const cfg = this.options.video || {};
    if (cfg.accept && cfg.accept !== 'video/*') {
      const ok = cfg.accept.split(',').some(a => {
        a = a.trim();
        return a.endsWith('/*') ? file.type.startsWith(a.slice(0, -1)) : file.type === a;
      });
      if (!ok) { this.emit('video:error', { file, reason: 'type' }); return; }
    }
    if (cfg.maxSize && file.size > cfg.maxSize) {
      this.emit('video:error', { file, reason: 'size' });
      return;
    }

    const makeVideo = (src) => {
      const VideoClass = this.registry.get('formats/video');
      if (VideoClass && typeof VideoClass.create === 'function') {
        return VideoClass.create(src); // <video controls> (or a YouTube iframe for YT urls)
      }
      return null;
    };
    ...
    if (typeof cfg.upload !== 'function') {
      ...
      reader.onload = (ev) => {
        const src = ev.target.result;
        // Mirror makeVideo's (Video.create's) scheme check on the raw fallback.
        const el = makeVideo(src);
        const html = el ? el.outerHTML
          : (isSafeUrl(src, { allowDataAV: true })
              ? `<video src="${src}" class="inserted-video" controls style="max-width:100%" contenteditable="false"></video>` : '');
        if (!html) return;
        this.focus();
        execFormat('insertHTML', html + '&nbsp;');
        this.onContentChange();
      };
      reader.onerror = () => this.emit('video:error', { file, reason: 'read' });
      reader.readAsDataURL(file);
      return;
    }

    const placeholderId = 'rte-vid-' + Math.round(performance.now()) + '-' + (this._vidCounter = (this._vidCounter || 0) + 1);
    const escName = (file.name || 'video')
      .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const phHTML =
      `<span class="yjd-upload" id="${placeholderId}" contenteditable="false" data-state="uploading">` +
      `<span class="yjd-spinner" aria-hidden="true"></span>` +
    ...
    Promise.resolve(cfg.upload(file)).then((res) => {
      ...
      if (url) {
        ...
        if (v) { el.replaceWith(v); } else { el.remove(); }
        this.emit('video:uploaded', { file, url });
      } else {
        el.remove();
      }
      this.onContentChange();
      ...
      const el = this.editor.querySelector('#' + placeholderId);
      if (el) el.remove();
      this.emit('video:error', { file, reason: 'upload', error: err });
      this.onContentChange();
    });
  };
  ...
  P.insertFileAttachment = function insertFileAttachment(file) {
    ...
    const makeChip = (url, meta = {}, state = '') => {
      ...
      // Scheme-check the chip href: the no-upload-hook path embeds the file as a
      // data: URL, so an attached .html would otherwise become a clickable
      // data:text/html link (script-capable). allowDataFile keeps inert embeds
      // (pdf/zip/office/text) while blocking the scriptable document types.
      a.setAttribute('href', safeChipHref(url));
    ...
    Promise.resolve(cfg.upload(file)).then((res) => {
      ...
      el.setAttribute('href', safeChipHref(url));
