...
export function applySerializeMethods(Editor) {
  ...
  Editor.prototype.streamMarkdown = function streamMarkdown() {
    ...
    // Insert the host at BLOCK level (never inside a <p>/<li>), so block output
    // (headings, lists, code) doesn't nest illegally. Replace an empty block at
    // the caret, else insert after the caret's block; fall back to append.
    const sel = window.getSelection();
    let anchor = null;
    if (sel && sel.rangeCount && this.editor.contains(sel.getRangeAt(0).startContainer)) {
      let n = sel.getRangeAt(0).startContainer;
      if (n.nodeType === 3) n = n.parentNode;
      anchor = n && n.closest ? n.closest('p,li,h1,h2,h3,h4,h5,h6,blockquote,pre,div') : null;
      if (anchor === this.editor) anchor = null;
    }
    if (anchor && anchor.parentNode) {
      const empty = !anchor.textContent.trim() && !anchor.querySelector('img,table,hr,video,iframe');
      if (empty) anchor.replaceWith(host);
