...
export default class Editor {
    ...
    this.editor.addEventListener('drop', (e) => {
      ...
      if (files.length) {
        ...
        if (skipped.length) {
          ...
          const kind = ext && ext.length <= 5 ? ext.toUpperCase() : 'These';
          this.showToast(
            skipped.length === files.length
              ? `${kind} files aren't enabled in this editor.`
              : `${skipped.length} file${skipped.length > 1 ? 's' : ''} skipped — type not enabled in this editor.`
          );
        }
        return;
      }
  ...
  t(key, fallback) {
    ...
    if (s && typeof s === 'object' && s[key] != null) return s[key];
    return fallback;
  }
  ...
  updateStatusbar() {
    ...
    if (this.statusbarEls.breadcrumb && this.options.features.breadcrumb) {
      ...
      if (!currentNode || !this.editor.contains(currentNode)) {
        this.statusbarEls.breadcrumb.textContent = 'Body';
      } else {
        ...
        if (block && /^H[1-6]$/.test(block.tagName)) {
          ...
          const labels = {
            P: 'Paragraph', LI: 'List item', BLOCKQUOTE: 'Quote',
            PRE: 'Code', TD: 'Table cell', TH: 'Table header'
          };
          const label = block ? (labels[block.tagName] || 'Paragraph') : 'Paragraph';
          this.statusbarEls.breadcrumb.textContent = 'Body › ' + label;
        }
      }
    }
