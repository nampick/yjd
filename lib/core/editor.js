...
export default class Editor {
  ...
  /**
   * Auto-linkify: if the text immediately before a collapsed caret ends with a
   * bare URL (http(s):// or www.), wrap it in an <a>. Called when the URL is
   * "committed" with a space/Enter. Skips text already inside a link.
   */
  linkifyBeforeCaret() {
    const sel = window.getSelection();
    ...
    // Put the caret right after the new link so the space/Enter lands outside it.
   ...
   * editable root), or the caret's containing block. Used by block-level
   * commands so the change is written into the content itself.
   */
  _getSelectedBlocks() {
    const sel = window.getSelection();
    if (!sel || !sel.rangeCount) return [];
  ...
  toggleFullscreen() {
    ...
    // Swap the toolbar button icon + label.
    const toolbar = this.getModule('toolbar');
   ...
   * attribute lives in the content (persists on export) rather than only on the
   * editor wrapper. Falls back to the editor-wide toggle when no block resolves.
   */
  toggleTextDirection() {
    ...
    const history = this.getModule('history');
    if (history && typeof history.saveBeforeFormat === 'function') history.saveBeforeFormat();
    // Toggle relative to the first block's current direction.
  ...
  handleToolbarClick(data) {
    ...
    const alwaysAllowedCommands = ['more', 'undo', 'redo', 'code-view', 'theme', 'text-direction', 'find', 'ai', 'fullscreen'];
