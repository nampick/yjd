...
export default class Editor {
  ...
  setContent(html) {
    ...
    if (this.options.plainText && html) {
      ...
      const toText = (n) => {
        ...
        // A mention chip flattens to its serialized token, not its label (#69).
        if (n.classList && n.classList.contains('mention') && n.getAttribute('data-token')) {
  ...
  getText() {
    ...
    const tokenOf = (el) =>
      (el.classList && el.classList.contains('mention') && el.getAttribute('data-token')) || null;
    ...
    // Rich mode: fast path unless a tokenized mention is present.
    if (!this.editor.querySelector('.mention[data-token]')) {
