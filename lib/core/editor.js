...
export default class Editor {
  ...
  setupEventListeners() {
    ...
    if (this.options.submit &&
      ...
      this.editor.addEventListener('keydown', (e) => {
        ...
        // In the prompt layout on a touch device, the on-screen keyboard's
        // return key must insert a NEWLINE (so users can write multi-line
        // messages, add list items, etc.) — submit is the send button. Enter
        // still sends on desktop and in non-prompt comment boxes.
        if (this.options.layout === 'prompt' && typeof window !== 'undefined' &&
  ...
  submitContent() {
    ...
    if (typeof handler === 'function') handler(this.getContent(), this);
