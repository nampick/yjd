...
export default class Editor {
  ...
  _contentForSubmit() {
    ...
    if (!ser) return content;
    const atts = this.getAttachments();
    if (!atts.length) return content;
    const toStr = typeof ser === 'function' ? ser : (att) => this._defaultSerializeAttachment(att);
    const tail = atts.map(toStr).filter((s) => typeof s === 'string' && s).join('');
    return tail ? content + tail : content;
  }

  /** Default HTML serialization for an attachment (prompt.serializeAttachments: true). */
  _defaultSerializeAttachment(att) {
    const esc = (s) => String(s == null ? '' : s)
      .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
    const src = esc(att.src || '');
    const name = esc((att.file && att.file.name) || att.kind || '');
    if (att.kind === 'image') return `<p><img src="${src}" alt="${name}"></p>`;
    return `<p><a href="${src}">${name || src}</a></p>`;
  }
  ...
  _syncPromptSendState() {
    ...
    if (tb && typeof tb.updateSendState === 'function') {
