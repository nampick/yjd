...
export default class Editor {
  ...
  _emitChange() {
    ...
    // Enable/disable the prompt-layout send button with content (no-op when the
    // toolbar has no send button).
    const _tb = this.getModule('toolbar');
    if (_tb && typeof _tb.updateSendState === 'function') {
      _tb.updateSendState(this.isEditorEmpty());
    }
  ...
  submitContent() {
    ...
    if (!cfg || this.isEditorEmpty()) return;
