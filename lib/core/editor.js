...
// Decode a `data:<mime>;base64,<data>` URL into a File so it can be handed to an
// upload hook (used by options.media.migrateDataUrls). Returns null for a
// non-base64 or malformed data URL, or when the platform lacks atob/File.
function dataUrlToFile(dataUrl, baseName) {
  if (typeof dataUrl !== 'string' || !/^data:[^,]*;base64,/i.test(dataUrl)) return null;
  if (typeof atob !== 'function' || typeof File !== 'function') return null;
  try {
...
export default class Editor {
  static DEFAULTS = {
    // UI 2.0 empty state — surfaces the slash menu (a default module).
    ...
    theme: 'inherit',
    // Popover placement: 'auto' (default) portals popovers to <body> with
    // position:fixed ONLY when a clipping/transforming ancestor would otherwise
    // clip them; 'fixed' always portals; 'wrapper' keeps the legacy in-editor
    // absolute placement. See _popupStrategy().
    popup: 'auto',
    height: 400,
    // '100%' fills the container — what a block-level widget is expected to do.
    // (Was a fixed 800px, which overflowed any narrower container; the old
    // viewport clamp made that intermittent by window size. #61)
    width: '100%',
    maxWidth: 1200,
    maxHeight: 800,
    content: null, // Default content for the editor
    autoFocus: true, // focus the editor after mount; false = never steal focus
    // Plain-text mode (#60): for chat/prompt/comment hosts that store text.
    // All formatting is off — UI, ⌘B/I/U/K, markdown input rules, linkify —
    // paste is forced plain, and getContent()/onChange/submit deliver TEXT
    // (newline-separated), so a plain-text store can never receive markup.
    plainText: false,
    features: {
      emoji: true,
      image: true,
  ...
  constructor(selector, options = {}) {
    ...
    // plainText implies plain paste — pasted markup must never survive.
    if (this.options.plainText) this.options.pasteAsPlainText = true;
  ...
  init() {
    ...
    // Initial `content` is set via innerHTML (not setContent), so run the opt-in
    // data-URL migration here too, once modules/events are wired.
    this._migrateDataUrls();
  }
  ...
  createStructure() {
    ...
    // The OBJECT form ({ mode?, colors?, strict?, … }, #85) carries content
    // design tokens; its colour scheme lives in `.mode`.
    this._applyThemeAttr(this._themeMode());
    ...
    // Display caps for inserted videos (options.video.maxHeight/maxWidth) — a
    // tall/portrait clip is capped on insert (default 360px via CSS) so it
    // doesn't blow out the frame; resize handles still let the user drag larger.
    const vidCfg = (this.options.video && typeof this.options.video === 'object') ? this.options.video : null;
    if (vidCfg && vidCfg.maxHeight != null) this.wrapper.style.setProperty('--rte-video-max-h', cssSize(vidCfg.maxHeight));
    if (vidCfg && vidCfg.maxWidth != null) this.wrapper.style.setProperty('--rte-video-max-w', cssSize(vidCfg.maxWidth));
    ...
    this.editor.setAttribute('data-placeholder', this.t('editor.placeholder', this.options.placeholder));

    // Accessibility: expose the editable region to assistive technology
    this.editor.setAttribute('role', 'textbox');
    this.editor.setAttribute('aria-multiline', 'true');
    this.editor.setAttribute('aria-label', this.options.ariaLabel || this.options.placeholder || this.t('editor.aria', 'Rich text editor'));
  ...
  loadModules() {
    ...
    if (this.options.plainText) {
      // Plain-text mode: no formatting surfaces (bubble, slash menu, side
      // panel, …) — just the bar and undo/redo. mention/ai are appended below
      // when configured (both round-trip plain text).
      modulesToLoad = this.options.modules || ['toolbar', 'history'];
    } else if (hasToolbarConfig) {
      // User wants custom toolbar - load only basic modules
      modulesToLoad = this.options.modules || ['toolbar', 'history'];
    } else {
      // No toolbar config - load full feature set
      modulesToLoad = this.options.modules || ['toolbar', 'history', 'block-toolbar', 'table-toolbar', 'code-view', 'code-block-tools', 'block-handles', 'side-panel', 'resize-handles', 'find-replace', 'slash-menu', 'mention', 'button', 'sections'];
    }
    ...
    // Template atoms (#78/#79): inert without declared items, so they load
    // only when configured — alongside any toolbar/plainText combination.
    if (this.options.variables && !modulesToLoad.includes('variables')) {
      modulesToLoad.push('variables');
    }
    if (this.options.blocks && !modulesToLoad.includes('blocks')) {
      modulesToLoad.push('blocks');
    }
    if (this.options.schema && !modulesToLoad.includes('schema')) {
      modulesToLoad.push('schema');
    }
    // Builder mode (#84 UX): the author-friendly palette + structure surface.
    if (this.options.builder && !modulesToLoad.includes('builder')) {
      modulesToLoad.push('builder');
      if (!modulesToLoad.includes('sections')) modulesToLoad.push('sections');
      if (!modulesToLoad.includes('button')) modulesToLoad.push('button');
      if (!modulesToLoad.includes('block-handles')) modulesToLoad.push('block-handles');
    }
  ...
  loadFormats() {
    ...
    if (this.options.plainText) {
      // Plain-text mode: no formats at all — nothing can mark up the content.
      formatsToLoad = [];
    } else if (hasToolbarConfig) {
  ...
  setupEventListeners() {
    ...
    // WebKit + IME (e.g. Vietnamese keyboards on iOS) can commit text through
    // paths whose mutations arrive too late for the send-state sync — text sits
    // in the prompt pill while the send button stays disabled. compositionend
    // is the one signal WebKit reliably emits when the IME commits; re-sync
    // there (and on focus, below) as a belt.
    ...
    // drop-zone highlight while files are dragged over the editor. A visible
    // drop caret tracks the pointer so the insertion point is legible before
    // release (the caret is placed there on drop either way).
    this.editor.addEventListener('dragover', (e) => {
      if (e.dataTransfer && Array.from(e.dataTransfer.types || []).includes('Files')) {
        e.preventDefault();
        this.editor.classList.add('yjd-drag-over');
        this._showDropCaret(e.clientX, e.clientY);
      }
    });
    ...
    // Handle drop events (drag and drop). Every matching file inserts — a
    // four-photo drop is four images, not the first one with three discarded.
    this.editor.addEventListener('drop', (e) => {
      ...
      if (files.length) {
        // insertImageFile / insertVideoFile / insertFileAttachment are optional
        // (all-in-one) capabilities; a Minimal build without them swallows the
        // drop instead of letting the browser navigate to the file — but says so.
        e.preventDefault();
        this.placeCaretAtPoint(e.clientX, e.clientY);
        const skipped = [];
        files.forEach((f) => {
          if (f.type && f.type.startsWith('image/') && this.insertImageFile) {
            this.insertImageFile(f);
          } else if (f.type && f.type.startsWith('video/') && this.insertVideoFile) {
            this.insertVideoFile(f);
          } else if (f && f.name && this.options.file && this.insertFileAttachment) {
            this.insertFileAttachment(f);
          } else {
            skipped.push(f);
          }
        });
        if (skipped.length) {
          const ext = (skipped[0].name || '').split('.').pop();
          const kind = ext && ext.length <= 5 ? ext.toUpperCase() : this.t('drop.these', 'These');
          this.showToast(
            skipped.length === files.length
              ? this.t('drop.kindNotEnabled', "{kind} files aren't enabled in this editor.", { kind })
              : this.t('drop.nSkipped', '{n} files skipped — type not enabled in this editor.', { n: skipped.length })
          );
        }
        return;
      }
    ...
    // plainText: both input rules stay off — "# " is content, not a command.
    if (!this.options.plainText && this.options.markdown !== false && this.handleMarkdownShortcut) {
      this.editor.addEventListener('keydown', (e) => {
    ...
    if (!this.options.plainText && this.options.linkify !== false && this.linkifyBeforeCaret) {
      this.editor.addEventListener('keydown', (e) => {
    ...
    // Enter on an EMPTY list item exits the list back to a paragraph (the
    // browser kept minting empty items instead — Notion/Docs convention).
    this.editor.addEventListener('keydown', (e) => {
      ...
      if (!li || !this.editor.contains(li) || li.textContent.trim() !== '') return;
      const list = li.parentElement;
      if (!list || !/^(UL|OL)$/.test(list.tagName)) return;
      // Nested list: let the browser outdent one level (its default is fine
      // there); only take over at the TOP level, where it mints empty items.
      if (list.parentElement && list.parentElement.closest('li')) return;
      // Mid-list empty item: splitting semantics are the browser's call —
      // only the trailing empty item means "I'm done with this list".
      if (li.nextElementSibling) return;
      e.preventDefault();
      const h = this.getModule('history');
    ...
    // Tab inside a LIST ITEM nests it (Shift+Tab outdents) — the browser
    // default tabbed focus out of the editor entirely. Table cells keep their
    // own Tab navigation (handled by the table module).
    this.editor.addEventListener('keydown', (e) => {
      ...
      if (node.nodeType === Node.TEXT_NODE) node = node.parentElement;
      const li = node && node.closest ? node.closest('li') : null;
      if (!li || !this.editor.contains(li)) return;
      // Inside a table cell WITHOUT a list, Tab moves cells — leave it alone.
      const cell = node.closest ? node.closest('td, th') : null;
      if (cell && !cell.contains(li)) return;
      e.preventDefault();
      const h = this.getModule('history');
      if (h && typeof h.saveBeforeFormat === 'function') h.saveBeforeFormat();
      execFormat(e.shiftKey ? 'outdent' : 'indent');
      this._emitChange();
    });

    // Enter inside a code block stays in the SAME <pre> (browsers split into a
    // new <pre> per line otherwise, which breaks multi-line code and the
    // header's language detection). Enter on an empty trailing line exits the
    // block into a fresh paragraph.
    this.editor.addEventListener('keydown', (e) => {
      if (e.key !== 'Enter' || e.isComposing || e.shiftKey || e.ctrlKey || e.metaKey || e.altKey) return;
      if (this.isMenuOpen && this.isMenuOpen()) return;
      const sel = window.getSelection();
      ...
      if (!pre || !this.editor.contains(pre) || pre.parentElement !== this.editor) return;
      e.preventDefault();
      ...
      // Exit when the caret sits on an empty trailing line: nothing on the
      // current line before it, and at most the render-helper "\n" after it.
      if (/^\n?$/.test(textAfter) && /(^|\n)$/.test(textBefore) && textBefore) {
        // Drop the blank trailing line(s) before leaving the block. Merge
        // adjacent text nodes first — the newline may sit in its own node.
        pre.normalize();
        const walker = document.createTreeWalker(pre, NodeFilter.SHOW_TEXT);
        let last = null;
        while (walker.nextNode()) last = walker.currentNode;
        if (last && /\n+$/.test(last.data)) last.data = last.data.replace(/\n+$/, '');
        const p = document.createElement('p');
        p.innerHTML = '<br>';
        pre.after(p);
        const r = document.createRange();
        ...
        // A lone trailing "\n" doesn't render a visible new line in white-space:
        // pre — double it when breaking at the very end of the block.
        const nl = document.createTextNode(!textAfter ? '\n\n' : '\n');
        range.insertNode(nl);
        const r = document.createRange();
        r.setStart(nl, 1);
        r.collapse(true);
    ...
    this.editor.addEventListener('focus', () => {
      ...
      setTimeout(() => {
        this.ensureEditorHasContent();
        this.updatePlaceholderVisibility();
        // IME belt (see compositionend above): focus is the other signal WebKit
        // emits reliably around IME writes that missed the mutation path.
        this._syncPromptSendState();
      }, 0);
    });
  }
  ...
  /**
   * Resolve a UI string. `options.strings` is either a flat map
   * `{ key: 'translation' }` or a function `(key, fallback) => string`; anything
   * missing (or a nullish function return) falls back to the built-in English
   * `fallback`. This is the single lookup every hardcoded label goes through, so
   * an integrator can localise the toolbar tooltips, popup titles, buttons and
   * attachment chrome without forking the library.
   *
   * `params` interpolates `{name}` placeholders in the resolved string (so a
   * composite like "Hide {n} more tools" localises as one unit):
   * `t('toolbar.hideN', 'Hide {n} more tools', { n: 3 })`.
   *
   * @param {string} key - stable dot-namespaced key (e.g. 'popup.insertLink')
   * @param {string} fallback - the built-in English string
  ...
  t(key, fallback, params) {
    ...
    if (typeof s === 'function') {
      const r = s(key, fallback);
      if (r != null) out = r;
    } else if (s && typeof s === 'object' && s[key] != null) {
      out = s[key];
    }
    if (params) {
      out = String(out).replace(/\{(\w+)\}/g, (m, k) => (params[k] != null ? params[k] : m));
    }
    return out;
  }
  ...
  /**
   * Update toolbar accessibility. UI 2.0: the bar stays ENABLED at rest —
   * greying every tool until the user clicks into the document (the old
   * outside-selection disable) made the editor look broken on first load.
   * Clicking a tool focuses the editor and applies at the caret / restored
   * selection. Read-only and code view keep their own disable paths.
   */
  updateToolbarAccessibility() {
    const toolbar = this.getModule('toolbar');
    if (!toolbar) return;
    if (this._readOnly) return; // read-only disable owns the state
    const codeView = this.getModule('code-view');
    if (codeView && codeView.isCodeView) return; // code view owns the state
    ...
    const editingCommands = [
      ...
      'more', 'undo', 'redo', 'code-view', 'theme'
    ];
  ...
  updateStatusbar() {
    ...
    if (this.statusbarEls.breadcrumb && this.options.features.breadcrumb) {
      ...
      if (!currentNode || !this.editor.contains(currentNode)) {
        this.statusbarEls.breadcrumb.textContent = this.t('status.body', 'Body');
      } else {
        // Friendly breadcrumb (UI 2.0): "H2 · Patch pipeline" inside a heading,
        // otherwise "Body › Paragraph" — not the raw CSS selector path.
        let element = currentNode?.nodeType === 3 ? currentNode.parentElement : currentNode;
        const block = element && element.closest
          ? element.closest('h1, h2, h3, h4, h5, h6, li, blockquote, pre, td, th, p')
          : null;
        if (block && /^H[1-6]$/.test(block.tagName)) {
          let title = (block.textContent || '').trim().replace(/\s+/g, ' ');
          if (title.length > 32) title = title.slice(0, 31) + '…';
          this.statusbarEls.breadcrumb.textContent =
            block.tagName + (title ? ' · ' + title : '');
        } else {
          const labels = {
            P: this.t('status.paragraph', 'Paragraph'), LI: this.t('status.listItem', 'List item'),
            BLOCKQUOTE: this.t('status.quote', 'Quote'), PRE: this.t('status.code', 'Code'),
            TD: this.t('status.tableCell', 'Table cell'), TH: this.t('status.tableHeader', 'Table header')
          };
          const label = (block && labels[block.tagName]) || this.t('status.paragraph', 'Paragraph');
          this.statusbarEls.breadcrumb.textContent = this.t('status.body', 'Body') + ' › ' + label;
        }
      }
    }
    ...
    if (this.statusbarEls.wordcount && this.options.features.wordCount) {
      ...
      if (codeView && typeof codeView.isInCodeView === 'function' && codeView.isInCodeView()) {
        ...
        // innerText = the VISIBLE text: collapsed whitespace, real newlines in
        // <pre>. textContent counted markup whitespace (e.g. source-view
        // pretty-print residue) that the user never sees.
        text = this.editor.innerText != null ? this.editor.innerText : (this.editor.textContent || '');
      }
      ...
      let label = this.t('status.wordCount', '{words} words, {chars} chars ({noSpaces} no spaces)',
        { words, chars, noSpaces: charsNoSpaces });
      if (this.options.maxLength) {
        label += this.t('status.charsLeft', ' • {n} left', { n: Math.max(0, this.options.maxLength - chars) });
      }
   ...
   */
  getContent() {
    // Plain-text mode: the host stores text, so the content API delivers text
    // (newline-separated blocks) — markup can never leak into a save path.
    if (this.options.plainText) return this.getText();
    const html = this.editor.innerHTML;
    // Find & Replace wraps matches in <mark class="yjd-find-hit"> as a transient
    // visual overlay, and variable chips / slot cards (#78/#79) are editor
    // chrome whose serialized form is their raw token. Both are stripped from
    // the output via a detached clone so the live DOM stays untouched.
    const hasAtoms = html.indexOf('yjd-var') !== -1 || html.indexOf('yjd-slot') !== -1;
    if (html.indexOf('yjd-find-hit') === -1 && !hasAtoms) return html;
    const tmp = document.createElement('div');
    tmp.innerHTML = html;
    ...
    if (hasAtoms) this._serializeAtoms(tmp);
    return tmp.innerHTML;
  }

  /**
   * Downgrade template atoms in a detached clone to their raw tokens (#78/#79):
   * a variable chip becomes its `{token}` text, a slot card becomes a
   * `<p>{{token}}</p>` paragraph. The stored document format stays plain
   * text + tokens — nothing proprietary survives serialization.
   */
  _serializeAtoms(root) {
    root.querySelectorAll('.yjd-var[data-token]').forEach((chip) => {
      chip.parentNode.replaceChild(document.createTextNode(chip.getAttribute('data-token')), chip);
    });
    root.querySelectorAll('.yjd-slot[data-token]').forEach((card) => {
      const p = document.createElement('p');
      p.textContent = card.getAttribute('data-token');
      card.parentNode.replaceChild(p, card);
    });
  }

  /**
   * Toggle variable preview (#78): chips display their sample values and the
   * editor is read-only while active, so authors read the mail as a recipient.
   */
  previewVariables(on) {
    const vars = this.getModule && this.getModule('variables');
    if (vars && typeof vars.setPreview === 'function') vars.setPreview(on);
  }

  /** Re-chip declared tokens after content lands in the DOM (#78/#79). */
  _upgradeAtoms() {
    const vars = this.getModule && this.getModule('variables');
    if (vars && typeof vars.upgrade === 'function') vars.upgrade(this.editor);
    const blocks = this.getModule && this.getModule('blocks');
    if (blocks && typeof blocks.upgrade === 'function') blocks.upgrade(this.editor);
    const btn = this.getModule && this.getModule('button');
    if (btn && typeof btn.restyleAll === 'function') btn.restyleAll();
    const secs = this.getModule && this.getModule('sections');
    if (secs && typeof secs.restyleAll === 'function') secs.restyleAll();
    const schema = this.getModule && this.getModule('schema');
    if (schema && typeof schema.filterTags === 'function') schema.filterTags(this.editor);
  }

  /**
   * Validate the document against options.schema (#81):
   * { valid, violations: [{rule, detail, message}] }. Always valid when no
   * schema is configured.
   */
  validate() {
    const m = this.getModule && this.getModule('schema');
    return m && typeof m.validate === 'function'
      ? m.validate() : { valid: true, violations: [] };
  }

  /**
   * Insert a CTA button block (#83). Props: { label, href, bg, color, radius,
   * padding, align, full }. No-op when the button module isn't loaded.
   */
  insertButton(props = {}) {
    const m = this.getModule && this.getModule('button');
    return m && typeof m.insert === 'function' ? m.insert(props) : null;
  }
   ...
   */
  setContent(html) {
    // Plain-text mode: markup handed in is flattened to text on the way in —
    // each block/newline becomes its own paragraph, tags never survive.
    if (this.options.plainText && html) {
      const tmp = document.createElement('div');
      tmp.innerHTML = sanitizeHtml(String(html));
      const esc = (t) => t.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      // Block-aware flatten: block elements end a line, inline elements flow
      // into it, <br> breaks it. Pure-text input keeps its own newlines.
      const BLOCK = /^(ADDRESS|ARTICLE|ASIDE|BLOCKQUOTE|DD|DIV|DL|DT|FIGCAPTION|FIGURE|FOOTER|H[1-6]|HEADER|HR|LI|MAIN|NAV|OL|P|PRE|SECTION|TABLE|TR|UL)$/;
      const toText = (n) => {
        if (n.nodeType === 3) return n.textContent;
        if (n.nodeType !== 1) return '';
        if (n.tagName === 'BR') return '\n';
        // A mention chip / slot card flattens to its serialized token, not its
        // visible label (#69, #79).
        if (n.classList && (n.classList.contains('mention') || n.classList.contains('yjd-slot')) &&
            n.getAttribute('data-token')) {
          return n.getAttribute('data-token');
        }
        const inner = [...n.childNodes].map(toText).join('');
        return BLOCK.test(n.tagName) ? inner + '\n' : inner;
      };
      const hasElements = [...tmp.childNodes].some((n) => n.nodeType === 1 && n.tagName !== 'BR');
      let text = [...tmp.childNodes].map(toText).join('').replace(/\u200B/g, '');
      if (hasElements) {
        text = text
          .replace(/[ \t]*\n[ \t]*/g, '\n') // formatting whitespace at breaks
          .replace(/\n{3,}/g, '\n\n')       // runs of blank lines \u2192 one
          .replace(/^\n+|\n+$/g, '');
      }
      const lines = text ? text.split('\n') : [];
      html = lines.length
        ? lines.map((l) => (l.trim() ? `<p>${esc(l)}</p>` : '<p><br></p>')).join('')
        : '';
    }
    ...
    // Declared {token} / {{slot}} strings in incoming content become chips and
    // cards again (#78/#79) — round-trip: setContent(getContent()) preserves
    // atoms exactly.
    this._upgradeAtoms();
    this.onContentChange();
    // Opt-in: re-upload any legacy data: media left in saved drafts (from before
    // the popup/attachment upload hooks existed) through the matching hook.
    this._migrateDataUrls();
  }

  /**
   * With options.media.migrateDataUrls, walk the just-loaded content for
   * <img>/<video> whose src is a data: URL and, when an upload hook is
   * configured, decode each to a File, run it through the hook, and swap the src
   * to the returned URL in place. Idempotent (a swapped element is http(s), no
   * longer data:) and best-effort (an element is left untouched if its hook is
   * absent or throws). This heals bodies persisted during the old base64 era so
   * they stop re-embedding megabytes on every save.
   */
  _migrateDataUrls() {
    const media = this.options.media;
    if (!media || !media.migrateDataUrls) return;
    const imgUp = this.options.image && this.options.image.upload;
    const vidUp = (this.options.video && this.options.video.upload) || imgUp;
    if (typeof imgUp !== 'function' && typeof vidUp !== 'function') return;

    const els = this.editor.querySelectorAll('img[src^="data:"], video[src^="data:"]');
    els.forEach((el) => {
      const isVideo = el.tagName === 'VIDEO';
      const up = isVideo ? vidUp : imgUp;
      if (typeof up !== 'function') return;
      const file = dataUrlToFile(el.getAttribute('src'), (isVideo ? 'video' : 'image'));
      if (!file) return;
      this.emit('media:migrate', { el, file });
      Promise.resolve(up(file)).then((res) => {
        const url = typeof res === 'string' ? res : (res && res.url);
        if (!url) return;
        el.setAttribute('src', url);
        this.emit('media:migrated', { el, url });
        this.onContentChange();
      }).catch((error) => {
        this.emit('media:migrate-error', { el, file, error });
      });
    });
  }
   ...
   */
  getText() {
    // A mention chip carries its serialized token in data-token; the TEXT of a
    // mention is that token, not the chip's visible label — a plain-text store
    // would otherwise save a corrupted tag (#69).
    const tokenOf = (el) =>
      (el.classList && (el.classList.contains('mention') || el.classList.contains('yjd-slot')) &&
        el.getAttribute('data-token')) || null;

    // Plain-text mode is paragraph-aware: block children separate with '\n'
    // and <br> is a newline — textContent alone would glue lines together.
    if (this.options.plainText) {
      // Text of one block: <br> inside a paragraph (Shift+Enter) is '\n';
      // zero-width placeholders are dropped; mention chips emit their token.
      const blockText = (el) => {
        let out = '';
        el.childNodes.forEach((n) => {
          if (n.nodeType === 3) out += n.textContent;
            ...
            const tok = tokenOf(n);
            out += tok != null ? tok : (n.tagName === 'BR' ? '\n' : blockText(n));
          }
        });
        return out.replace(/\u200B/g, '');
      };
      const lines = [...this.editor.childNodes].map((node) => {
        if (node.nodeType === 3) return (node.textContent || '').replace(/\u200B/g, '');
        if (node.nodeType !== 1) return '';
        if (node.tagName === 'BR') return '';
        // <p><br></p> (the empty-line shape this editor maintains) → ''.
        const t = blockText(node);
        return t === '\n' ? '' : t;
      });
      // A trailing empty <p><br></p> is the resting caret line, not content.
      while (lines.length && lines[lines.length - 1] === '') lines.pop();
      return lines.join('\n');
    }
    // Rich mode: fast path unless a tokenized mention/slot is present.
    if (!this.editor.querySelector('.mention[data-token], .yjd-slot[data-token]')) {
    ...
    const walk = (el) => {
      ...
      el.childNodes.forEach((n) => {
        if (n.nodeType === 3) out += n.textContent;
          ...
          const tok = tokenOf(n);
          out += tok != null ? tok : walk(n);
        }
      });
      return out;
    };
    return walk(this.editor);
  }
   ...
   */
  _contentForSubmit() {
    // plainText content is text, so its attachment tail is the markdown shape
    // (`![](src)` / `[name](url)`) rather than HTML tags.
    const tail = this._serializeAttachmentsTail(this.options.plainText ? 'markdown' : 'html');
    const content = this.getContent();
  ...
  _defaultSerializeAttachment(att, format) {
    ...
    // Scheme-check the attachment URL with the same allowlist the serializer and
    // DOM sanitizer use (a raster data URL is fine for an image, an AV data URL
    // for a video); an unsafe src is dropped so it can't ride into the folded
    // content as a javascript:/data:text/html link.
    const src = isSafeUrl(att.src || '', {
      allowDataImage: att.kind === 'image',
      allowDataAV: att.kind === 'video',
    }) ? String(att.src).trim() : '';
    if (format === 'markdown') {
  ...
  /** The library stylesheet text (as injected by StylesLoader), '' if absent. */
  _stylesText() {
    const el = document.getElementById('rich-editor-styles');
    return el ? el.textContent : '';
  }

  /** Document HTML: current content rendered with the read-view class + tokens. */
  _exportDocument(title = 'Document') {
    const esc = (s) => String(s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
    return '<!doctype html><html><head><meta charset="utf-8">' +
      `<title>${esc(title)}</title>` +
      `<style>${this._stylesText()}</style>` +
      '<style>body{margin:2rem auto;max-width:720px;padding:0 1rem;' +
      'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif}</style>' +
      `</head><body class="yjd-content">${this.getHTML()}</body></html>`;
  }

  /**
   * Print the document (UI 2.0 `print` command): renders the content with the
   * read-view styles in a hidden iframe and opens the print dialog.
   */
  printContent() {
    ...
    doc.open();
    doc.write(this._exportDocument(document.title || 'Document'));
    doc.close();
    const win = frame.contentWindow;
    const cleanup = () => setTimeout(() => frame.remove(), 500);
    frame.onload = () => {
      try { win.focus(); win.print(); } catch (e) { /* blocked */ }
      cleanup();
    };
    // Some browsers fire load synchronously after close().
    if (doc.readyState === 'complete') { try { win.focus(); win.print(); } catch (e) { /* noop */ } cleanup(); }
  }

  /**
   * Download the document (UI 2.0 `download` command).
   * @param {'html'|'md'} format
   */
  downloadContent(format = 'html') {
    const md = format === 'md' && typeof this.getMarkdown === 'function';
    const data = md ? this.getMarkdown() : this._exportDocument(document.title || 'Document');
    const blob = new Blob([data], { type: md ? 'text/markdown' : 'text/html' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = md ? 'document.md' : 'document.html';
  ...
  handlePaste(e) {
    ...
    // Pasted video file → inline <video> player (same path as drag-drop).
    const videoItem = items.find(it => it.kind === 'file' && it.type && it.type.startsWith('video/'));
    if (videoItem && this.insertVideoFile) {
      const file = videoItem.getAsFile();
      if (file) {
        e.preventDefault();
        this.insertVideoFile(file);
        return;
      }
    }
  ...
  /**
   * Visible drop caret: a 2px accent bar at the caret position under the
   * pointer during a file drag, so the exact insertion point shows before
   * release. Reuses the same caret-from-point lookup as placeCaretAtPoint.
   */
  _showDropCaret(x, y) {
    let range = null;
    if (document.caretRangeFromPoint) {
    ...
    if (!range || !this.editor.contains(range.startContainer)) { this._hideDropCaret(); return; }
    range.collapse(true);
    let rect = range.getBoundingClientRect();
    if ((!rect || (!rect.height && !rect.width)) && range.startContainer.nodeType === 1) {
      rect = range.startContainer.getBoundingClientRect();
    }
    if (!rect || !rect.height) { this._hideDropCaret(); return; }
    if (!this._dropCaret) {
      this._dropCaret = document.createElement('div');
      this._dropCaret.className = 'yjd-drop-caret';
      document.body.appendChild(this._dropCaret);
    }
    this._dropCaret.style.left = `${Math.round(rect.left)}px`;
    this._dropCaret.style.top = `${Math.round(rect.top)}px`;
    this._dropCaret.style.height = `${Math.round(rect.height)}px`;
  }
  ...
  /**
   * Transient toast anchored to the viewport bottom — used where an action
   * would otherwise fail silently (e.g. a dropped file type with no handler).
   */
  showToast(message, { duration = 3200 } = {}) {
    if (this._toast) this._toast.remove();
    const t = document.createElement('div');
    t.className = 'yjd-toast';
    t.setAttribute('role', 'status');
    // Portaled to <body>: copy theme vars from the wrapper so a themed editor
    // themes its toast too (same pattern as the AI / slash / mention chrome).
    try {
      const cs = getComputedStyle(this.wrapper);
      ['--rte-bg', '--rte-ink-2', '--rte-border-strong', '--rte-radius-md', '--rte-shadow', '--rte-warning', '--rte-ui']
        .forEach((v) => { const val = cs.getPropertyValue(v); if (val) t.style.setProperty(v, val.trim()); });
    } catch (e) { /* non-DOM env */ }
    t.innerHTML =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z"></path><path d="M12 9v4"></path><path d="M12 17h.01"></path></svg>' +
      '<span></span>';
    t.querySelector('span').textContent = String(message || '');
    document.body.appendChild(t);
    this._toast = t;
    this._toastTimer = setTimeout(() => {
      if (this._toast === t) { t.remove(); this._toast = null; }
    }, duration);
    return t;
  }
  ...
  /**
   * Insert an inline date chip at the caret (UI 2.0 `date` command).
   * @param {Date} [date] - defaults to today; stored on data-date as ISO.
   */
  insertDateChip(date = new Date()) {
    const historyModule = this.getModule('history');
    if (historyModule && typeof historyModule.saveBeforeFormat === 'function') {
      historyModule.saveBeforeFormat();
    }
    ...
    chip.className = 'yjd-date-chip';
    chip.setAttribute('data-date', date.toISOString().slice(0, 10));
    chip.setAttribute('contenteditable', 'false');
    chip.textContent = date.toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' });
  ...
  setTheme(theme) {
    ...
    // The object form (#85) keeps its design tokens — only the mode changes.
    if (this.options.theme && typeof this.options.theme === 'object') this.options.theme.mode = t;
    else this.options.theme = t;
    this._applyThemeAttr(t);
    this.emit('theme:change', t);
    return this;
  }

  /** The colour-scheme part of options.theme, whatever its form. */
  _themeMode() {
    const th = this.options.theme;
    if (th && typeof th === 'object') return th.mode || 'inherit';
    return th || 'inherit';
  }
  ...
  handleToolbarClick(data) {
    ...
    const alwaysAllowedCommands = ['more', 'send', 'undo', 'redo', 'code-view', 'theme', 'text-direction', 'find', 'ai', 'fullscreen', 'print', 'download'];
    ...
    if (alwaysAllowedCommands.includes(command)) {
      ...
      switch (command) {
        ...
        case 'print':
          ...
          return;
        case 'download':
          this.downloadContent(data && data.value === 'md' ? 'md' : 'html');
          return;
      }
    }
    ...
    switch (command) {
      ...
      case 'date':
        ...
        break;
      case 'quote': {
        // Toggle: back to a paragraph when the caret is already in a quote.
        const sel2 = window.getSelection();
        let n = sel2 && sel2.rangeCount ? sel2.getRangeAt(0).startContainer : null;
        if (n && n.nodeType === 3) n = n.parentNode;
        const inQuote = !!(n && n.closest && n.closest('blockquote') && this.editor.contains(n));
        this.setBlockType(inQuote ? 'p' : 'blockquote');
        break;
      }
      default:
  ...
  toggleFormat(formatName) {
    // Plain-text mode: formatting is a no-op everywhere — this central guard
    // covers the ⌘B/I/U/K shortcuts and any hand-configured toolbar button.
  ...
  updateToolbarButtonStates() {
    ...
    formats.forEach(formatName => {
      // Allow-list guard (#59): only query formats whose control actually
      // exists in the toolbar. Instantiating a format builds its picker popup
      // into the DOM as a side effect, so probing all 19 used to mount every
      // dropdown (heading levels, font list, line heights, …) even for a
      // toolbar restricted to e.g. ['undo', 'redo'].
      if (!toolbar.getButton(formatName)) return;
   ...
   */
  getPopupContainer() {
    // In 'fixed' strategy, popovers portal to a body-level themed root so a host
    // ancestor's overflow/transform/stacking can't clip or bury them.
    if (this._popupStrategy() === 'fixed') return this._getPortalRoot();
    return this.popupContainer;
  }

  /**
   * Popover placement strategy:
   *  - 'wrapper' (legacy): absolute, inside the editor (scoped, multi-instance
   *    clean, but hostage to a host ancestor's overflow/transform/z-index).
   *  - 'fixed': portal to document.body + position:fixed (immune to host
   *    clipping/stacking; the tradeoff is it can overlay host chrome).
   *  - 'auto' (default): 'fixed' when a clipping/transforming ancestor sits
   *    between the editor and <body> (the case that breaks in host apps),
   *    otherwise 'wrapper'. Cached after first resolve.
   */
  _popupStrategy() {
    const opt = this.options.popup;
    if (opt === 'fixed' || opt === 'wrapper') return opt;
    if (this._popupStrategyCache) return this._popupStrategyCache;
    let strat = 'wrapper';
    try {
      // Walk ancestors between the wrapper and <body> (exclusive): a scroll
      // container, transform, filter, perspective or paint/layout containment
      // there would clip or re-root an absolute popover. html/body scrolling is
      // fine (the whole editor moves with it), so we stop before them.
      const CLIP = new Set(['hidden', 'auto', 'scroll', 'clip', 'overlay']);
      let el = this.wrapper && this.wrapper.parentElement;
      while (el && el !== document.body && el !== document.documentElement) {
        const cs = getComputedStyle(el);
        // Only a REAL clipping value counts (an empty computed value, as some
        // headless DOMs return, is treated as non-clipping — never a false
        // 'fixed').
        const clips = CLIP.has(cs.overflowX) || CLIP.has(cs.overflowY);
        const has = (v) => v && v !== 'none';
        const reRoots = has(cs.transform) || has(cs.filter) || has(cs.perspective) ||
          (cs.contain && /paint|layout|strict|content/.test(cs.contain));
        if (clips || reRoots) { strat = 'fixed'; break; }
        el = el.parentElement;
      }
    } catch (e) { /* SSR / detached — keep 'wrapper' */ }
    this._popupStrategyCache = strat;
    return strat;
  }

  /**
   * Lazily create a body-level portal root that carries the editor's theme class
   * and tokens, so portaled popovers keep their yjd styling (the CSS is scoped
   * under `.yjd-rich-editor`) and per-instance theme/token overrides.
   */
  _getPortalRoot() {
    if (this._portalInner && this._portalInner.isConnected) return this._portalInner;
    const root = document.createElement('div');
    root.className = 'yjd-rich-editor yjd-popup-portal';
    // Mirror the instance theme + any inline --rte-* token overrides so the
    // portaled popcovers render identically to the in-wrapper ones.
    const theme = this.wrapper && this.wrapper.getAttribute('data-theme');
    if (theme) root.setAttribute('data-theme', theme);
    const st = this.wrapper && this.wrapper.style;
    if (st) {
      for (let i = 0; i < st.length; i++) {
        const name = st[i];
        if (name && name.indexOf('--rte-') === 0) root.style.setProperty(name, st.getPropertyValue(name));
      }
    }
    // Popups mount in an INNER '.rich-editor-popup-container', not on the
    // portal root itself: the popup styling is scoped with descendant
    // selectors (`.yjd-rich-editor .rich-editor-popup-container …`), so the
    // two classes must sit on two nested elements exactly like the in-wrapper
    // layout. With both classes compounded on one element those rules never
    // matched, and host-page CSS (e.g. a global `select { font-size: … }`)
    // bled into portaled popups past the base-layer font enforcement —
    // overflowing the fixed-height tag-popup select. (#67)
    const inner = document.createElement('div');
    inner.className = 'rich-editor-popup-container';
    root.appendChild(inner);
    document.body.appendChild(root);
    this._portalRoot = root;
    this._portalInner = inner;
    return inner;
  }
   ...
   */
  destroy() {
    // Remove the body-level popover portal, if this instance created one.
