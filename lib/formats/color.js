...
class Color extends InlineFormat {
  ...
  constructor() {
    ...
    if (!colorPicker) {
      ...
      const ed = Editor.getCurrentInstance();
      colorPicker = new ColorPicker({
        title: ed && typeof ed.t === 'function' ? ed.t('color.text', 'Text colour') : 'Text colour',
        onColorSelect: (color) => {
  ...
  static applyColorToCurrentSelection(color, editorId = null) {
    ...
    // Focus the editor FIRST — execCommand('foreColor') only applies/arms on a
    // focused contentEditable, and clicking a swatch left focus on the picker.
    const edInst = editorId != null ? Editor.getInstanceById(editorId) : Editor.getCurrentInstance();
    if (edInst && edInst.editor && document.activeElement !== edInst.editor) {
      edInst.editor.focus();
    }
    ...
    } else if (edInst && typeof edInst.restoreSelectionToEditor === 'function') {
      // No saved range (e.g. empty editor) — drop a caret into the editor.
      edInst.restoreSelectionToEditor();
    }
    if (editorId != null) Color.savedRanges.delete(editorId);

    // Allow a collapsed caret: foreColor on a collapsed selection arms the
    // colour for the next typed character (works on an empty/just-focused
    // editor), exactly like Bold. Only bail when there's no range at all.
    ...
    if (selection.isCollapsed) {
      // Collapsed caret: arm the colour as a real DOM placeholder that also
      // carries any bold/italic/underline/strike the user just armed. Using
      // execCommand('foreColor') here would wipe those pending formats.
      insertArmedColorSpan(selection, 'color', colorVal);
    } else {
      setStyleWithCSS(true);
      execFormat('foreColor', colorVal);
      // styleWithCSS is a DOCUMENT-WIDE sticky flag: left on, every later
      // bold/italic serializes as <span style> instead of <b>/<i>.
      setStyleWithCSS(false);
    }

    // Remember the armed colour for a collapsed caret so the toolbar swatch shows
    // it immediately (before any text is typed). Cleared on input/selection move.
    if (selection.isCollapsed) {
      const edInst = editorId != null ? Editor.getInstanceById(editorId) : Editor.getCurrentInstance();
      if (edInst) edInst._armedFore = color === 'transparent' ? null : color;
    }
