...
registerIcons({
  'text-size': S('<path d="M8 5H3v14"/><path d="M3 12h4"/><path d="M13 9h8"/><path d="M17 9v10"/><path d="M14 19h6"/>')
});
  ...
  apply(size = '4') {
    ...
    if (!range.collapsed) {
        ...
        if (sel.rangeCount > 0) {
            ...
            // Nếu container là text node → normalize ở parent (nếu còn gắn DOM)
            if (container.nodeType === Node.TEXT_NODE) {
                if (container.parentNode) container.parentNode.normalize();
            } else {
                container.normalize();
            }
        }   
