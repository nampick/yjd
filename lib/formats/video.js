...
registerIcons({
  video: S('<rect x="2" y="5" width="14" height="14" rx="2"/><path d="m16 10 6-3v10l-6-3z"/>')
});
...
class Video extends InlineFormat {
  ...
  static create(src) {
    ...
    // Reject unsafe URL schemes for direct (non-YouTube) video sources. Uploaded
    // clips arrive as data:video/* URLs (inert media), so allow those too.
    if (!isSafeUrl(src, { allowDataAV: true })) {
  ...
  showVideoPopup() {
    ...
    if (toolbar) {
      videoButton = toolbar.getButton('video') || toolbar.getButton('add');
    }
