...
registerIcons({
  'text-align': S('<line x1="21" x2="3" y1="6" y2="6"/><line x1="15" x2="3" y1="12" y2="12"/><line x1="17" x2="3" y1="18" y2="18"/>'),
...
class TextAlign extends BlockFormat {
  ...
  static applyAlignmentToBlock(block, alignment) {
    // Set every alignment explicitly (incl. 'left'). Clearing it made the block
    // fall back to the area's inherited `text-align: start`, which renders RIGHT
    // in an RTL context — so "Align left" could show right-aligned.
  ...
  static updateToolbarButtonIcon(alignment, editorId = null) {
    ...
    // Update button title (localisable via 'align.<value>')
    const ed = Editor.getInstanceById(this.editorId);
    const enTitle = titleMap[alignment] || 'Text Alignment';
    button.title = (ed && typeof ed.t === 'function') ? ed.t('align.' + alignment, enTitle) : enTitle;
