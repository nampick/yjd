...
class Background extends InlineFormat {
  ...
  constructor() {
    ...
    if (!colorPicker) {
      ...
      colorPicker = new ColorPicker({
        title: typeof currentEditor.t === 'function' ? currentEditor.t('color.background', 'Background') : 'Background',
        onColorSelect: (color) => {
  ...
  static applyBackgroundToCurrentSelection(color, editorId = null) {
    ...
    // The user SELECTED text and picked a colour, but the live selection got
    // collapsed on the way (focus churn between picker and editor) — falling
    // through to the armed-placeholder branch would light the button with no
    // visible highlight. Recover the last real selection instead.
    if (selection.isCollapsed && editor && editor._lastRange && !editor._lastRange.collapsed &&
        editor.editor.contains(editor._lastRange.startContainer)) {
      selection.removeAllRanges();
      selection.addRange(editor._lastRange.cloneRange());
    }
