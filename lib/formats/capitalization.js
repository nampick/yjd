...
class Capitalization extends InlineFormat {
  ...
  /**
   * Nearest block element containing the caret (never the editable root).
   * @param {Node} node
   * @returns {Element|null}
   */
  getCaretBlock(node) {
    ...
    while (n && n !== document.body) {
      if (n.nodeType === Node.ELEMENT_NODE) {
        if (n.classList && n.classList.contains('rich-editor-area')) return null;
  ...
  apply(style = 'none') {
    ...
    // No selection (empty editor or just a caret): a destructive per-character
    // rewrite has nothing to act on, so apply the case as a CSS text-transform on
    // the caret's block. This makes the option work on an empty editor — text
    // typed next inherits the transform — and lets it toggle ('none' clears it).
    if (range.collapsed) {
      const block = this.getCaretBlock(range.startContainer);
      if (block) this.setElementCapitalizationStyle(block, style);
      return;
    }
