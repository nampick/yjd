...
class Capitalization extends InlineFormat {
  ...
  updateButtonText() {
    ...
    if (capitalizationButton && capitalizationButton.updateText) {
      ...
      // v2: the toolbar renders this as an ICON button — only update the text
      // when a select-trigger .button-text exists (custom toolbar configs).
      const textEl = capitalizationButton.querySelector('.button-text');
      if (textEl) textEl.textContent = displayName;
      else capitalizationButton.title = displayName;
    }
  }
  ...
  apply(style = 'none') {
    ...
    // No selection (empty editor or just a caret): ARM the case like the other
    // inline text-style tools (font/size/colour) — materialise a placeholder
    // span carrying the style with the caret inside, so the NEXT typed text is
    // transformed and existing text is left untouched. (A collapsed caret has no
    // text to transform, and changing the whole line would be inconsistent with
    // every other character tool.) Works on an empty editor too.
    if (range.collapsed) {
      if (style === 'small-caps') insertArmedColorSpan(selection, 'fontVariant', 'small-caps');
      else insertArmedColorSpan(selection, 'textTransform', style || 'none');
      return;
    }
    ...
    // Remember a wrapping capitalization span (e.g. text-transform:uppercase)
    // BEFORE we extract, so we can clear it afterwards. The transform below is
    // destructive (it rewrites the characters), so any leftover CSS
    // text-transform would override the result — that's the "lowercase doesn't
    // take effect" bug when the text was previously upper/lower-cased via CSS.
    const ancestorCap = this.findAncestorCapitalizationElement(range.commonAncestorContainer);

    // Nếu có selection: đổi text bên trong
    const contents = range.extractContents();

    // Strip conflicting case CSS on any descendant elements in the selection so
    // the transformed characters render as-is.
    if (contents.querySelectorAll) {
      contents.querySelectorAll('*').forEach((el) => {
        if (el.style) { el.style.textTransform = ''; el.style.fontVariant = ''; }
      });
    }
    ...
    // Clear the wrapping span's case CSS too (it sits outside the extracted
    // fragment), so an inherited text-transform no longer hides the change.
    if (ancestorCap && ancestorCap.style) {
      ancestorCap.style.textTransform = '';
      ancestorCap.style.fontVariant = '';
    }
