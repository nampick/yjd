...
export default class Mention extends Module {
  ...
  _buildSources() {
    ...
    if (typeof cfg.source === 'function') {
      map[cfg.trigger || '@'] = { source: cfg.source, renderItem: cfg.renderItem || renderItem, serialize: cfg.serialize };
    }
    (cfg.triggers || []).forEach((t) => {
      if (t && t.char && typeof t.source === 'function') {
        map[t.char] = { source: t.source, renderItem: t.renderItem || renderItem, serialize: t.serialize };
      }
    });
    return map;
  }
    ...
    this.items.forEach((item, i) => {
      ...
      if (typeof renderItem === 'function') {
        ...
        if (item.avatar_url && isSafeUrl(item.avatar_url, { allowDataImage: true })) {
          ...
          // UI 2.0 fallback: initial avatar with a colour hashed from the name,
          // so rows read like the design even without avatar images.
          const ini = document.createElement('span');
          ini.className = 'yjd-mention-initial';
          ini.textContent = label[0].toUpperCase();
          const palette = ['#5b5bd6', '#0f8a5f', '#c2571c', '#1f6fdc', '#8b2fa0', '#a86500'];
          let hash = 0;
          for (let ci = 0; ci < label.length; ci++) hash = (hash * 31 + label.charCodeAt(ci)) | 0;
          ini.style.background = palette[Math.abs(hash) % palette.length];
          el.appendChild(ini);
        }
        // UI 2.0 two-line row: name on top, optional handle/description below
        // (from item.handle / item.hint / item.description — all app data).
        const sub = item.handle || item.hint || item.description || '';
        const textWrap = document.createElement('span');
        textWrap.className = 'yjd-mention-text';
        const nameEl = document.createElement('span');
        nameEl.className = 'yjd-mention-name';
        nameEl.textContent = sub ? label : this.char + label;
        textWrap.appendChild(nameEl);
        if (sub) {
          const subEl = document.createElement('span');
          subEl.className = 'yjd-mention-handle';
  ...
  choose(index) {
    ...
    try {
      ...
      // Per-trigger serialize(item) → the exact token this mention serializes to
      // (getMarkdown/getContent). Lets an integrator store e.g. a bare "#t-42"
      // instead of the default "#[Title](id)" without post-processing by regex.
      const trigCfg = this.sources[this.char];
      if (trigCfg && typeof trigCfg.serialize === 'function') {
        try {
          const tok = trigCfg.serialize(item);
          if (typeof tok === 'string') span.setAttribute('data-token', tok);
        } catch (e) { /* fall back to the default token */ }
      }
      del.insertNode(span);
