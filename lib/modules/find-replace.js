...
registerIcons({
  find: S('<circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/>')
});
...
export default class FindReplace extends Module {
  ...
  buildPanel() {
    ...
    // Three rows (UI 2.0): find controls, replace controls, match-option chips.
    const findRow = document.createElement('div');
    findRow.className = 'yjd-find-row';
    const replaceRow = document.createElement('div');
    replaceRow.className = 'yjd-find-row';
    const optsRow = document.createElement('div');
    optsRow.className = 'yjd-find-row yjd-find-opts';

    const t = (k, fb) => this.t('find.' + k, fb);
    this.findInput = mkInput(t('find', 'Find'), 'yjd-find-field');
    this.replaceInput = mkInput(t('replace', 'Replace with'), 'yjd-find-field');
    this.countEl = document.createElement('span');
    this.countEl.className = 'yjd-find-count';
    this.countEl.textContent = '0/0';

    this.prevBtn = mkBtn('', t('prev', 'Previous match'), 'yjd-find-icon', 'chevron-up');
    this.nextBtn = mkBtn('', t('next', 'Next match'), 'yjd-find-icon', 'chevron-down');
    this.caseBtn = mkBtn('Aa', t('matchCase', 'Match case'), 'yjd-find-toggle');
    this.wordBtn = mkBtn(t('wholeWord', 'Whole word'), t('wholeWordHint', 'Match whole words only'), 'yjd-find-toggle');
    this.regexBtn = mkBtn('.*', t('regex', 'Regular expression'), 'yjd-find-toggle yjd-find-regex');
    this.closeBtn = mkBtn('', t('close', 'Close (Esc)'), 'yjd-find-icon yjd-find-close', 'close');
    this.replaceBtn = mkBtn(t('replaceBtn', 'Replace'), t('replaceCurrent', 'Replace current'));
    this.replaceAllBtn = mkBtn(t('replaceAll', 'Replace all'), t('replaceAllHint', 'Replace all matches'));

    findRow.append(this.findInput, this.countEl, this.prevBtn, this.nextBtn, this.closeBtn);
    replaceRow.append(this.replaceInput, this.replaceBtn, this.replaceAllBtn);
    optsRow.append(this.caseBtn, this.wordBtn, this.regexBtn);
    panel.append(findRow, replaceRow, optsRow);
  ...
  bindEvents() {
    ...
    const bindToggle = (btn, prop) => btn.addEventListener('click', () => {
      this[prop] = !this[prop];
      btn.classList.toggle('active', this[prop]);
      btn.setAttribute('aria-pressed', this[prop] ? 'true' : 'false');
      this.runSearch();
    });
    bindToggle(this.caseBtn, 'caseSensitive');
    bindToggle(this.wordBtn, 'wholeWord');
    bindToggle(this.regexBtn, 'useRegex');
  ...
  runSearch() {
    ...
    let regex;
    try {
      // Regex mode uses the term verbatim; otherwise escape it. Whole-word
      // wraps either form in word boundaries (regex authors can still use \b
      // themselves — double boundaries are harmless).
      let src = this.useRegex ? term : this.escapeRegex(term);
      if (this.wholeWord) src = `\\b(?:${src})\\b`;
      regex = new RegExp(src, this.caseSensitive ? 'g' : 'gi');
    } catch (e) {
      this.updateCount();
      return;
    }
