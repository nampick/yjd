...
export default class BlockHandles extends Module {
  ...
  _bind() {
    this._onMove = (e) => {
      ...
      // Only direct children of the editable area count as draggable blocks.
      while (block && block.parentElement !== this.editor.editor) {
    ...
    this.dragBtn.addEventListener('pointerdown', (e) => {
      ...
      const onMove = (ev) => {
        const blocks = Array.from(this.editor.editor.children).filter((el) => el !== block);
        let before = null;
      ...
      const onUp = () => {
        ...
        if (changed) {
          const h = this.editor.getModule('history');
          if (h && typeof h.saveBeforeFormat === 'function') h.saveBeforeFormat();
          if (before) this.editor.editor.insertBefore(block, before);
  ...
  _position() {
    ...
    const left = Math.max(2, blockRect.left - wrapRect.left - 40);
    this.gutter.style.left = `${Math.round(left)}px`;
    this.gutter.style.top = `${Math.round(blockRect.top - wrapRect.top + 2)}px`;
