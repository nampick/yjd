...
class TableToolbar extends Module {
  ...
  /** Prompt for a colour and apply it as the cell background (empty = clear). */
  setCellBackground(button) {
    const cells = this.targetCells();
    if (!cells.length) return;
    const current = cells[0].style.backgroundColor || '';
    const val = window.prompt('Cell background colour (hex/name, empty to clear):', current);
    if (val === null) return; // cancelled
    this._beforeEdit();
    cells.forEach((c) => { c.style.backgroundColor = val.trim(); });
    this._afterEdit();
  }
