...
registerIcons({
  'table-profile': S('<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/>'),
  'add-row-above': S('<rect x="3" y="11" width="18" height="10" rx="2"/><path d="M12 3v5"/><path d="M9.5 5.5 12 3l2.5 2.5"/>'),
  'add-row-below': S('<rect x="3" y="3" width="18" height="10" rx="2"/><path d="M12 21v-5"/><path d="M9.5 18.5 12 21l2.5-2.5"/>'),
  'add-col-left': S('<rect x="11" y="3" width="10" height="18" rx="2"/><path d="M3 12h5"/><path d="M5.5 9.5 3 12l2.5 2.5"/>'),
  'add-col-right': S('<rect x="3" y="3" width="10" height="18" rx="2"/><path d="M21 12h-5"/><path d="M18.5 9.5 21 12l-2.5 2.5"/>'),
  'delete-row': S('<rect x="3" y="4" width="18" height="10" rx="2"/><path d="m9.5 18.5 5 5"/><path d="m14.5 18.5-5 5"/>'),
  'delete-col': S('<rect x="9" y="3" width="6" height="18" rx="2"/><line x1="12" x2="12" y1="9" y2="15"/>'),
  'delete-table': S('<path d="M4 6h16"/><path d="M6 6l1 14h10l1-14"/><path d="M10 11v5"/><path d="M14 11v5"/>'),
  'cell-merge': S('<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 12h6"/><path d="M15 12h6"/>'),
  'cell-split': S('<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M12 3v18"/><path d="m9.5 9.5 2.5 2.5 2.5-2.5"/>'),
  'header-col': S('<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 3v18"/>'),
  'cell-bg': S('<path d="m9 11-6 6v3h9l3-3"/><path d="m22 12-4.6 4.6a2 2 0 0 1-2.8 0l-5.2-5.2a2 2 0 0 1 0-2.8L14 4"/><rect x="3" y="3" width="8" height="8" rx="1"/>'),
  'align-left': S('<path d="M3 6h18"/><path d="M3 12h11"/><path d="M3 18h15"/>'),
  'align-center': S('<path d="M3 6h18"/><path d="M6.5 12h11"/><path d="M4.5 18h15"/>'),
  'align-right': S('<path d="M3 6h18"/><path d="M10 12h11"/><path d="M6 18h15"/>')
});
...
class TableToolbar extends Module {
  static DEFAULTS = {
    ...
    buttons: ['tableProfile', 'toggleHeaderRow', 'toggleHeaderCol', 'deleteTable', 'insertRowAbove', 'insertRowBelow', 'deleteRow', 'insertColRight', 'insertColLeft', 'deleteCol', 'cellAlignLeft', 'cellAlignCenter', 'cellAlignRight', 'cellBg', 'mergeCells', 'splitCell']
  };
    ...
    const buttonGroups = [
      {
        name: 'table-actions',
        buttons: [
          { cmd: 'tableProfile', icon: 'icon-table-profile', title: this.t('table.profile', 'Table Profile') },
          { cmd: 'toggleHeaderRow', icon: 'icon-heading', title: this.t('table.toggleHeaderRow', 'Toggle Header Row') },
          { cmd: 'toggleHeaderCol', icon: 'icon-header-col', title: this.t('table.toggleHeaderCol', 'Toggle Header Column') },
          { cmd: 'deleteTable', icon: 'icon-delete-table', title: this.t('table.delete', 'Delete Table') }
        ]
      },
      {
        name: 'row-actions',
        buttons: [
          { cmd: 'insertRowAbove', icon: 'icon-add-row-above', title: this.t('table.rowAbove', 'Add Row Above') },
          { cmd: 'insertRowBelow', icon: 'icon-add-row-below', title: this.t('table.rowBelow', 'Add Row Below') },
          { cmd: 'deleteRow', icon: 'icon-delete-row', title: this.t('table.deleteRow', 'Delete Selected Row') }
        ]
      },
      {
        name: 'col-actions',
        buttons: [
          { cmd: 'insertColRight', icon: 'icon-add-col-right', title: this.t('table.colRight', 'Add Column Right') },
          { cmd: 'insertColLeft', icon: 'icon-add-col-left', title: this.t('table.colLeft', 'Add Column Left') },
          { cmd: 'deleteCol', icon: 'icon-delete-col', title: this.t('table.deleteCol', 'Delete Selected Column') }
        ]
      },
      {
        name: 'cell-actions',
        buttons: [
          { cmd: 'cellAlignLeft', icon: 'icon-align-left', title: this.t('table.cellLeft', 'Align cell left') },
          { cmd: 'cellAlignCenter', icon: 'icon-align-center', title: this.t('table.cellCenter', 'Align cell center') },
          { cmd: 'cellAlignRight', icon: 'icon-align-right', title: this.t('table.cellRight', 'Align cell right') },
          { cmd: 'cellBg', icon: 'icon-cell-bg', title: this.t('table.cellBg', 'Cell background colour') },
          { cmd: 'mergeCells', icon: 'icon-cell-merge', title: this.t('table.merge', 'Merge selected cells') },
          { cmd: 'splitCell', icon: 'icon-cell-split', title: this.t('table.split', 'Split cell') }
        ]
      }
    ];
  ...
  ensureToolbarInViewport(x, y, width, height) {
    ...
    if (arrow) {
      ...
      if (arrowDirection === 'up') {
        ...
        arrow.style.borderBottom = '8px solid var(--rte-bg, #fff)';
        ...
        arrow.style.borderTop = '8px solid var(--rte-bg, #fff)';
        arrow.style.borderLeft = '6px solid transparent';
        arrow.style.borderRight = '6px solid transparent';
      }
    }
  ...
  setCellBackground(button) {
    ...
    if (!this.cellColorPicker) {
      this.cellColorPicker = new ColorPicker({
        title: this.t('table.cellBgTitle', 'Cell background'),
        editor: this.editor,
        onColorSelect: (color) => {
   ...
   * the design's `header-col` sibling of the header-row toggle (UI 2.0).
   */
  toggleHeaderCol() {
    const table = this.currentTable;
    ...
    const firstCells = Array.from(table.rows).map((r) => r.cells[0]).filter(Boolean);
    if (!firstCells.length) return;
    const makeHeader = firstCells.some((c) => c.tagName !== 'TH');
  ...
  showTableProfile() {
    ...
    const items = [
      [this.t('table.rows', 'Rows'), rows], [this.t('table.columns', 'Columns'), cols], [this.t('table.cells', 'Cells'), cells],
      [this.t('table.width', 'Width'), Math.round(t.offsetWidth) + 'px'], [this.t('table.height', 'Height'), Math.round(t.offsetHeight) + 'px']
    ];
    ...
    el.innerHTML = `<div class="yjd-table-profile-title">${this.t('table.title', 'Table')}</div>` +
  ...
  createNewCell() {
    ...
    cell.style.border = '1px solid var(--rte-table-border, #dfe2e7)';
