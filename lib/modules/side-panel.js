 ...
 *  `t` is an optional translator (editor.t) so the stamps localise. */
function relTime(ts, t) {
  const tr = t || ((k, fb) => fb);
  const d = Date.now() - ts;
  const m = Math.floor(d / 60000);
  if (m < 1) return tr('panel.timeNow', 'now');
  if (m < 60) return tr('panel.timeMin', '{n}m', { n: m });
  const h = Math.floor(m / 60);
  if (h < 24) return tr('panel.timeHour', '{n}h', { n: h });
  const days = Math.floor(h / 24);
  return days === 1 ? tr('panel.timeYest', 'yest') : tr('panel.timeDay', '{n}d', { n: days });
}
...
export default class SidePanel extends Module {
  constructor(editor, options = {}) {
    ...
    this.user = (this.cfg.user && this.cfg.user.name) || this.t('panel.you', 'You');
  ...
  _build() {
    ...
    this.tabs.forEach((t) => {
      ...
      b.textContent = this.t('panel.tab.' + t, LABELS[t] || t);
  ...
  _threadRow(author, body, time) {
    ...
    t.textContent = relTime(time, (k,fb,p)=>this.t(k,fb,p));
    head.append(who, t);
  ...
  openCommentComposer() {
    ...
    field.rows = 3;
    field.placeholder = this.t('panel.commentPlaceholder', 'Comment…');
    ...
    cancel.textContent = this.t('cancel', 'Cancel');
    const submitBtn = document.createElement('button');
    submitBtn.type = 'button';
    submitBtn.className = 'yjd-button-confirm yjd-c-cbtn';
    submitBtn.textContent = this.t('panel.comment', 'Comment');
    ...
    hint.textContent = this.t('panel.composerHint', '⌘⏎ submits · Esc discards');
  ...
  _renderBody(focusCommentId) {
    ...
    if (this.activeTab === 'outline') {
      ...
      if (!rows.length) return this._empty(this.t('panel.noHeadings', 'No headings yet'));
      rows.forEach((r) => {
        ...
        row.querySelector('.yjd-side-title').textContent = r.text || this.t('panel.untitled', '(untitled)');
      ...
      if (!this.versions.length) return this._empty(this.t('panel.noVersions', 'No versions — call saveVersion()'));
      this.versions.forEach((v, i) => {
        ...
        const liveWords = i === 0
          ...
          : v.words;
        row.querySelector('.yjd-side-vwho').textContent = this.t('panel.words', '{n} words', { n: liveWords });
        const tag = row.querySelector('.yjd-side-vtag');
        if (i === 0) tag.textContent = this.t('panel.current', 'Current');
  ...
  _renderComments(b, focusCommentId) {
    ...
    [['open', this.t('panel.filterOpen', 'Open · {n}', { n: open.length })], ['resolved', this.t('panel.filterResolved', 'Resolved · {n}', { n: resolved.length })]].forEach(([key, label]) => {
    ...
    if (!list.length) {
      return this._empty(this._commentFilter === 'resolved'
        ? this.t('panel.noneResolved', 'Nothing resolved yet')
        : this.t('panel.noOpenComments', 'No open comments — select text and press ⌘⌥M'));
    }
    ...
    list.forEach((c) => {
      if (c.resolved) {
        ...
        row.innerHTML =
          ...
          `<button type="button" class="yjd-c-undo">${this.t('panel.undo', 'Undo')}</button>`;
        row.querySelector('.yjd-c-resolved-text').textContent =
          this.t('panel.resolvedBy', 'Resolved by {who} — "{quote}"', { who: c.resolvedBy || '?', quote: c.body.slice(0, 40) });
      ...
      const t = document.createElement('span');
      t.className = 'yjd-c-time';
      t.textContent = relTime(c.time, (k,fb,p)=>this.t(k,fb,p));
      ...
      if (c.replies && c.replies.length) {
        ...
        rep.textContent = c.replies.length > 1
          ? this.t('panel.repliesN', '{n} replies', { n: c.replies.length })
          : this.t('panel.reply1', '{n} reply', { n: c.replies.length });
      ...
      if (orphan) {
        ...
        o.innerHTML =
          '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0"></path><path d="M12 9v4"></path><path d="M12 17h.01"></path></svg><span></span>';
        o.querySelector('span').textContent = this.t('panel.anchorDeleted', 'Anchor text was deleted');
        card.appendChild(o);
      }
