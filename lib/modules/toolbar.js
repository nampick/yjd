...
class Toolbar extends Module {
  ...
  reflow() {
    ...
    // UI 2.0 v2: ONE horizontally-scrollable overflow row (design's counted
    // overflow) with a small "⌄ OVERFLOW" label — never a multi-row stack.
    const row = document.createElement('div');
    row.className = 'rich-editor-toolbar-2-row yjd-noscroll';
    row.style.paddingLeft = padL + 'px';
    row.style.paddingRight = padR + 'px';
    const label = document.createElement('span');
    label.className = 'rte-overflow-label';
    label.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg>Overflow';
  ...
  _syncMoreButton() {
    ...
    if (this.toolbar2Visible) {
      ...
      m.title = n ? `Hide ${n} more tools` : 'Hide more options';
    } else {
      m.classList.remove('active');
      m.classList.remove('open');
      m.title = n ? `Show ${n} more tools` : 'More options';
    }
  }
  ...
  addButton(container, format) {
    ...
    const customButtons = {
      'heading': { text: 'Paragraph', width: '112px', title: 'Paragraph style' },
      'font-family': { text: 'Font Family', width: '106px', title: 'Font' },
      ...
      'text-size': { text: 'Text Size', width: '58px', title: 'Font size' }
    };
    ...
    const iconButtons = {
      'text-align': { icon: 'text-align', title: 'Text align' },
