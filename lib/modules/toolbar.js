...
class Toolbar extends Module {
  ...
  constructor(editor, options = {}) {
    ...
    if (_wantPrompt) {
      ...
      const tools = Array.isArray(promptCfg.tools) ? promptCfg.tools : ['bold', 'italic', 'link'];
      this.options = {
        container: null,
        toolbar1: [
          { group: 'add', items: ['add'] },
   ...
   */
  _resolveAddItems() {
    const cfg = (this.editor.options.prompt && this.editor.options.prompt.add) || ['image', 'file'];
    const out = [];
    cfg.forEach((entry) => {
