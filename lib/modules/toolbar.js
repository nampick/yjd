...
class Toolbar extends Module {
  ...
  constructor(editor, options = {}) {
    ...
    const _wantPrompt = options.toolbar === 'prompt' ||
      (_promptLayout && options.toolbar == null && !options.toolbar1 && !options.toolbar2);
  ...
  addAddButton(container) {
    ...
    button.addEventListener('click', async (e) => {
      ...
      // A single add action runs directly — no popover. This is the default
      // (add image) and the mobile-friendly behaviour: tapping "+" opens the
      // file picker straight away. Configure two+ items to get the menu.
      if (actionable.length <= 1) {
        if (actionable[0]) this._onAddSelect(actionable[0]);
        return;
      }
      if (!this._addMenu) {
  ...
  static ADD_ITEMS = {
    image: { label: 'Add image', icon: 'image', command: 'image' },
    file:  { label: 'Add file',  icon: 'file',  command: 'file' },
    video: { label: 'Add video', icon: 'video', command: 'video' },
   ...
   * image + file when nothing is configured.
   */
  _resolveAddItems() {
    const cfg = (this.editor.options.prompt && this.editor.options.prompt.add) || ['image'];
    const out = [];
    cfg.forEach((entry) => {
