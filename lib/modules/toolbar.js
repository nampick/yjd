...
class Toolbar extends Module {
  static DEFAULTS = {
    container: null,
    toolbar1: [
      // Clusters mirror the design's chrome bar (hairline dividers in CSS):
      // history · block picker · inline formatting incl. colours ·
      // lists/quote/link · insert. Alignment + indents move to the overflow
      // row like the design's Standard preset.
      { group: 'history', items: ['undo', 'redo'] },
      { group: 'paragraph', items: ['heading'] },
      { group: 'text-format', items: ['bold', 'italic', 'underline', 'strike', 'code', 'color', 'background'] },
      // The design's single "+" replaces separate image/table buttons — it
      // opens the insert menu (image / file / video / table by default).
      { group: 'paragraph-ops', items: ['list', 'quote', 'link', 'add'] },
      // The AI group (auto-injected when a model is wired) and the "more"
      // toggle sit on the right — the AI group carries margin-left:auto.
      { group: 'more', items: ['more'] }
    ],
    toolbar2: [
      { group: 'align', items: ['text-align', 'indent-increase', 'indent-decrease'] },
      { group: 'font', items: ['font-family', 'text-size', 'line-height'] },
      { group: 'script', items: ['subscript', 'superscript', 'capitalization', 'letter-spacing'] },
  ...
  constructor(editor, options = {}) {
    ...
    if (aiCfg && typeof aiCfg.complete === 'function') {
      ...
      if (!present) {
        const t1 = this.options.toolbar1 || [];
        const aiGroup = { group: 'ai', items: ['ai'] };
        const moreIdx = t1.findIndex((g) => g.items && g.items.includes('more'));
        this.options.toolbar1 = moreIdx >= 0
          ? [...t1.slice(0, moreIdx), aiGroup, ...t1.slice(moreIdx)]
          : [...t1, aiGroup];
      }
    }
  ...
  createToolbarContainer() {
    ...
    merged.forEach(group => {
      ...
      // UI 2.0: the history group (undo/redo) leads the row on the left like
      // every other flow group — it's first in the default config, so it's the
      // highest-priority group and never overflows into the hidden panel.
      // The AI group is pinned to the right edge instead (next to "More") and
      // never overflows: the Ask AI pill is a primary affordance, so it stays
      // visible at every width. reflow() reserves its room via pinnedGroup.
      if (group.group === 'ai') {
  ...
  reflow() {
    ...
    // the sibling margin and the overflow rows' flex gap), so a themed editor
    ...
    const ROW_GAP = this._gapToken(); // matches .rich-editor-toolbar-2-row's gap (same token)
    let row = null;
