...
class BlockToolbar extends Module {
  static DEFAULTS = {
    ...
    showOnEnter: false,
    // Input-like fields (#75): show the bubble when the editor gains focus —
    // anchored at the caret — and keep it through collapsed selections until
    // focus leaves the field. Select-to-reveal still works alongside it.
    showOnFocus: false,
    // The "⋮" overflow sheet (#70): true = all entries, false = no sheet,
    // or an allow-list of entry ids: 'highlight' | 'comment' | 'copy' |
    // 'clear-format'.
    sheet: true,
  ...
  createBlockToolbar() {
    ...
    const meta = {
      bold: { icon: 'bold', title: this.t('block.bold', 'Bold (Ctrl+B)') },
      italic: { icon: 'italic', title: this.t('block.italic', 'Italic (Ctrl+I)') },
      underline: { icon: 'underline', title: this.t('block.underline', 'Underline (Ctrl+U)') },
      strike: { icon: 'strike', title: this.t('block.strike', 'Strikethrough') },
      code: { icon: 'code', title: this.t('block.code', 'Code') },
      'font-family': { icon: 'font-family', title: this.t('block.fontFamily', 'Font Family') },
      link: { icon: 'link', title: this.t('block.link', 'Insert link') },
      color: { icon: 'color', title: this.t('block.color', 'Text color') },
      background: { icon: 'background', title: this.t('block.background', 'Background color') }
    };
    ...
    // Entries are built-in command names OR custom actions (#74):
    //   { id, title, icon?: registered-name | '<svg…>', text?, onClick(editor, selection) }
    const buttons = names.map((entry) => {
      if (entry && typeof entry === 'object') return entry;
      return { cmd: entry, icon: (meta[entry] && meta[entry].icon) || entry, title: (meta[entry] && meta[entry].title) || entry };
    });
    ...
    if (this.editor.options.ai && typeof this.editor.options.ai.complete === 'function') {
      ...
      aiBtn.title = this.t('block.editWithAi', 'Edit with AI');
      aiBtn.innerHTML =
    ...
    buttons.forEach((entry) => {
      // Custom action (#74): same .block-toolbar-btn chrome, app-owned click.
      if (entry && typeof entry.onClick === 'function') {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'block-toolbar-btn block-toolbar-custom';
        if (entry.title) { btn.title = entry.title; btn.setAttribute('aria-label', entry.title); }
        if (entry.id) btn.dataset.command = entry.id;
        if (entry.icon && /^\s*<svg/i.test(entry.icon)) {
          btn.innerHTML = entry.icon;
        } else if (entry.icon) {
          btn.appendChild(IconUtils.createIconElement(entry.icon, { width: '16px', height: '16px' }));
        } else {
          const span = document.createElement('span');
          span.textContent = entry.text || entry.id || '?';
          btn.appendChild(span);
        }
        // Don't blur the editor (keeps the selection the handler receives).
        btn.addEventListener('pointerdown', (e) => e.preventDefault());
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          entry.onClick(this.editor, window.getSelection());
        });
        toolbarContainer.appendChild(btn);
        return;
      }
      const { cmd, icon, title } = entry;
    ...
    // core four fold into a scrollable sheet under the bubble. options.sheet
    // filters it (#70): false = no ⋮ at all (and no dangling separator), an
    // array = only those entry ids; empty result also drops the ⋮.
    const sheetItems = this._sheetItems();
    if (sheetItems.length) {
      ...
      more.title = this.t('block.moreTools', 'More tools');
      more.innerHTML =
  ...
  setupEventListeners() {
    ...
    // NOTE: no window-scroll listener. The bubble is an absolutely positioned
    // child of the wrapper with element-relative coordinates, so it rides page
    // scroll for free — repositioning on window scroll double-counted the
    // offset and made the bubble drift away from its field (#76). The editor's
    // INTERNAL scroll still repositions (the selection moves within the
    // wrapper there).
    ...
    this._onEditorKeyup = (e) => {
      // Shift + Enter hides the toolbar (not in focus mode — the bubble is the
      // field's toolbar there and stays until blur).
      if (e.key === 'Enter' && e.shiftKey && !this.options.showOnFocus) {
        this.hide();
        return;
      }
    ...
    // Input-like fields (#75): the bubble appears on focus, anchored at the
    // caret, and hides only when focus leaves the field (and the bubble).
    if (this.options.showOnFocus) {
      this._onFocusIn = () => this._showAtCaretOrField();
      this._onFocusOut = (e) => {
        const to = e.relatedTarget;
        if (to && (this.editor.wrapper.contains(to) ||
          (this.blockToolbar && this.blockToolbar.contains(to)))) return;
        this.hide();
      };
      this.editor.editor.addEventListener('focusin', this._onFocusIn);
      this.editor.editor.addEventListener('focusout', this._onFocusOut);
    }
  ...
  handleSelectionChange() {
    ...
    } else if (this.options.showOnFocus && this._editorFocused()) {
      // Input-like mode (#75): a collapsed caret keeps the bubble — it follows
      // the caret instead of vanishing the moment the selection collapses.
      if (this.isVisible) this.updateToolbarPosition();
      else this._showAtCaretOrField();
    } else {
      this.hide();
    }
  }
  ...
  _editorFocused() {
    ...
    return !!a && (a === this.editor.editor || this.editor.editor.contains(a));
  }

  /**
   * showOnFocus anchor (#75): the caret when it has a rect, otherwise the top
   * of the editable area (a freshly focused empty field has no caret rect).
   */
  _showAtCaretOrField() {
    const wrapRect = this.editor.wrapper.getBoundingClientRect();
    let rect = null;
    const sel = window.getSelection();
    if (sel && sel.rangeCount && this.editor.editor.contains(sel.getRangeAt(0).startContainer)) {
      const r = sel.getRangeAt(0).getBoundingClientRect();
      if (r && (r.width || r.height)) rect = r;
    }
    if (!rect) {
      const area = this.editor.editor.getBoundingClientRect();
      rect = { left: area.left + 16, top: area.top + 4, bottom: area.top + 22, width: 0 };
    }
    this.showAt(
      rect.left + (rect.width || 0) / 2 - wrapRect.left,
  ...
  showAtSelection(selection) {
    ...
    // Wrapper-relative anchor, NO page-scroll terms: the bubble is an absolute
    // child of the wrapper, so these coordinates are scroll-independent (#76).
  ...
  showAt(x, y, yBottom) {
    ...
    this.isVisible = true;
    this.ensureToolbarInViewport(x, y, yBottom);
    this.updateButtonStates();
  }

  /**
   * Position the bubble relative to a WRAPPER-relative anchor.
   *
   * (x, yTop, yBottom): x is the anchor's horizontal centre, yTop/yBottom the
   * anchor's top/bottom edge — all relative to the editor wrapper, with NO
   * page-scroll terms. The bubble is an absolutely positioned child of the
   * wrapper, so element-relative coordinates are scroll-independent by
   * construction: it stays pinned to its field through any page scrolling
   * (#76). It may freely OVERHANG the editor above or below — a short,
   * input-sized field gets a floating bubble instead of none at all (#73),
   * and a top chrome toolbar no longer eats the headroom (#72). The bubble
   * only flips below the anchor when above would poke out of the VIEWPORT.
   */
  ensureToolbarInViewport(x, yTop, yBottom) {
    if (!this.blockToolbar) return;
    if (yBottom == null) yBottom = yTop + 18;

    const wrap = this.editor.wrapper;
    const wrapRect = wrap.getBoundingClientRect();
    const w = this.blockToolbar.offsetWidth;
    const h = this.blockToolbar.offsetHeight;
    const GAP = 10;

    // Prefer floating ABOVE the anchor; flip below only when the viewport cuts
    // it off (editor near the top of the page). Never hide.
    let top = yTop - h - GAP;
    let arrowDirection = 'down';
    if (wrapRect.top + top < 4) {
      top = yBottom + GAP;
      arrowDirection = 'up';
    }

    // Horizontal: clamp inside the wrapper; keep the arrow pointing at x.
    let left = x - w / 2;
    const maxLeft = wrap.offsetWidth - w - 2;
    if (left < 2) left = 2;
    if (left > maxLeft) left = Math.max(2, maxLeft);
    const arrowPct = w > 0 ? Math.min(92, Math.max(8, ((x - left) / w) * 100)) : 50;
    const arrowLeft = arrowPct + '%';
  ...
  updateToolbarPosition() {
    ...
    const yTop = rect.top - editorRect.top;
    const yBottom = rect.bottom - editorRect.top;

    this.updateToolbarAt(x, yTop, yBottom);
   ...
   */
  updateToolbarAt(x, yTop, yBottom) {
    if (!this.blockToolbar) return;

    this.ensureToolbarInViewport(x, yTop, yBottom);
    this.updateButtonStates();
  }
   ...
   */
  /**
   * The sheet's entry list after applying options.sheet (#70): true keeps all,
   * false drops all, an array keeps only the named ids (separators are kept
   * only between surviving entries).
   */
  _sheetItems() {
    const cfg = this.options.sheet;
    if (cfg === false) return [];
    const all = [
      {
        id: 'highlight',
        label: this.t('block.highlight', 'Highlight'), kbd: '',
      ...
      {
        id: 'comment',
        label: this.t('block.comment', 'Comment'), kbd: '⌘⌥M',
      ...
      {
        id: 'copy',
        label: this.t('block.copy', 'Copy'), kbd: '',
      ...
      {
        id: 'clear-format',
        label: this.t('block.clearFormat', 'Clear formatting'), kbd: '', danger: true,
    ...
    if (Array.isArray(cfg)) {
      const keep = new Set(cfg);
      items = all.filter((it) => it.separator || keep.has(it.id));
    }
    // Drop separators that no longer sit between two entries.
    return items.filter((it, i) => !it.separator ||
      (items.slice(0, i).some((x) => !x.separator) && items.slice(i + 1).some((x) => !x.separator)));
  }
  ...
  destroy() {
    ...
    if (this._onDocMousedown) {
      ...
      if (this._onFocusIn) this.editor.editor.removeEventListener('focusin', this._onFocusIn);
      if (this._onFocusOut) this.editor.editor.removeEventListener('focusout', this._onFocusOut);
      this._onDocMousedown = this._onEditorMouseup = null;
