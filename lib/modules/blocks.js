...
export default class Blocks extends Module {
  ...
  upgrade(root) {
    ...
    // Candidates are top-level blocks AND blocks inside section columns (#84)
    // — a slot token pasted into a column must upgrade the same way.
    const candidates = [
      ...root.children,
      ...root.querySelectorAll(':scope .yjd-col > *'),
    ];
    candidates.forEach((child) => {
      if (child.classList && (child.classList.contains('yjd-slot') ||
        child.classList.contains('yjd-section') || child.classList.contains('yjd-col'))) return;
      ...
      for (const m of matchers) {
        ...
        if (hit) {
          ...
          if (card && child.parentNode) child.parentNode.replaceChild(card, child);
          break;
        }
      }
    });
