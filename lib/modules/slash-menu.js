...
export default class SlashMenu extends Module {
  ...
  buildCommands() {
    ...
    if (ed.options.ai && typeof ed.options.ai.complete === 'function') {
      cmds.push({ id: 'ai', label: 'Ask AI…', hint: 'Edit or write with AI', icon: 'ai', run: () => this._askAi() });
    }
    return cmds.concat([
      { id: 'h1', label: 'Heading 1', hint: 'Big section heading', icon: 'heading', key: '#', run: () => ed.setBlockType('h1') },
      { id: 'h2', label: 'Heading 2', hint: 'Medium heading', icon: 'heading-2', key: '##', run: () => ed.setBlockType('h2') },
      { id: 'h3', label: 'Heading 3', hint: 'Small heading', icon: 'heading-3', key: '###', run: () => ed.setBlockType('h3') },
      { id: 'ul', label: 'Bullet list', hint: 'Unordered list', icon: 'list-bullet', key: '-', run: () => ed.setBlockType('ul') },
      { id: 'ol', label: 'Numbered list', hint: 'Ordered list', icon: 'list-ordered', key: '1.', run: () => ed.setBlockType('ol') },
      { id: 'quote', label: 'Quote', hint: 'Blockquote', icon: 'blockquote', key: '>', run: () => ed.setBlockType('blockquote') },
      { id: 'code', label: 'Code block', hint: 'Preformatted code', icon: 'code-block', key: '```', run: () => ed.setBlockType('pre') },
      { id: 'callout', label: 'Callout', hint: 'Highlighted note', icon: 'callout', run: () => this.insertCallout() },
      { id: 'toggle', label: 'Toggle', hint: 'Collapsible section', icon: 'toggle', run: () => this.insertToggle() },
      { id: 'hr', label: 'Divider', hint: 'Horizontal rule', icon: 'horizontal-rule', key: '---', run: () => ed.insertHorizontalRule() },
      { id: 'table', label: 'Table', hint: '3×3 table', icon: 'table', run: () => this.insertTable() },
      { id: 'p', label: 'Text', hint: 'Plain paragraph', icon: 'paragraph', run: () => ed.setBlockType('p') }
    ]);
  }
  ...
  _applyTheme() {
    ...
    ['--rte-bg', '--rte-chrome-2', '--rte-ink', '--rte-muted', '--rte-faint', '--rte-border', '--rte-border-strong', '--rte-accent', '--rte-accent-ink', '--rte-accent-weak', '--rte-radius-md', '--rte-radius-sm', '--rte-radius-xs', '--rte-shadow', '--rte-mono']
