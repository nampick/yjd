...
class CodeView extends Module {
  ...
  showNormalView() {
    ...
    // Drop the source view's pretty-print indentation: whitespace-only text
    // nodes BETWEEN block elements would otherwise stay in the document,
    // inflating the character count and the exported HTML. (Never touches
    // <pre> or whitespace inside inline runs.)
    const BLOCK = /^(P|DIV|H[1-6]|UL|OL|LI|TABLE|THEAD|TBODY|TR|TD|TH|BLOCKQUOTE|PRE|HR|DETAILS|FIGURE)$/;
    const walker = document.createTreeWalker(editorArea, NodeFilter.SHOW_TEXT);
    const junk = [];
    const blocky = (n) => !n || (n.nodeType === 1 && BLOCK.test(n.tagName));
    while (walker.nextNode()) {
      const t = walker.currentNode;
      if (t.parentElement && t.parentElement.closest('pre')) continue;
      // Indentation runs collapse to a single space in rendering anyway; fold
      // them so the character count matches what the user sees.
      t.data = t.data.replace(/[\t\n\r]+/g, ' ').replace(/ {2,}/g, ' ');
      // Trim the edges that touch a block boundary (start/end of a BLOCK
      // element, or beside a sibling block element) — pure pretty-print
      // residue. Edges inside INLINE elements keep their space (`a<b> b</b>`).
      const parentBlock = t.parentElement && BLOCK.test(t.parentElement.tagName);
      const prevBlocky = t.previousSibling ? blocky(t.previousSibling) : parentBlock;
      const nextBlocky = t.nextSibling ? blocky(t.nextSibling) : parentBlock;
      if (prevBlocky) t.data = t.data.replace(/^ /, '');
      if (nextBlocky) t.data = t.data.replace(/ $/, '');
      if (!t.data) junk.push(t);
    }
    ...
    // The statusbar counted the raw HTML while the source view was up —
    // recount against the restored document text.
    if (typeof this.editor.updateStatusbar === 'function') this.editor.updateStatusbar();
  }
  ...
  disableEditorEvents() {
    ...
    editorArea.title = this.t('code.disabledHint', 'Editor is disabled in code view mode. Click "Switch to Visual Editor" to return to normal editing.');
  }
  ...
  updateToolbarButton() {
    ...
    if (toolbar) {
      ...
      // Update button title (localisable; must not clobber a translated tooltip)
      const buttonTitle = this.isCodeView
        ? this.t('toolbar.codeViewExit', 'Switch to Visual Editor')
        : this.t('toolbar.code-view', 'Switch to HTML Editor');
  ...
  setContent(html) {
    if (this.isCodeView && this.codeTextarea) {
      ...
      // Sanitize even on this direct path — a caller could hand it raw HTML
      // (defense-in-depth; the primary write paths already sanitize).
      this.editor.editor.innerHTML = sanitizeHtml(html);
    }
  }
