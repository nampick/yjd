...
const THEME_VARS = ['--rte-accent', '--rte-accent-ink', '--rte-accent-weak', '--rte-accent-ring', '--rte-ink', '--rte-ink-2', '--rte-muted', '--rte-faint', '--rte-border', '--rte-border-strong', '--rte-bg', '--rte-chrome', '--rte-chrome-2', '--rte-radius-md', '--rte-radius-sm', '--rte-radius-xs', '--rte-shadow', '--rte-mono', '--rte-ui', '--rte-danger', '--rte-danger-weak', '--rte-success', '--rte-success-weak'];
...
export default class Ai extends Module {
  ...
  _build() {
    ...
    bar.innerHTML =
      '<div class="yjd-ai-head">Edit selection</div>' +
      ...
      '<form class="yjd-ai-ask">' +
        ...
        'placeholder="Ask AI…" aria-label="Ask AI">' +
    ...
    if (this.diffMode) {
      ...
      db.innerHTML =
        '<span class="yjd-ai-diffhint">Click a word to keep/drop · ⏎ accept · esc reject</span>' +
        '<button type="button" class="yjd-ai-accept" data-act="apply">Accept</button>' +
        '<button type="button" class="yjd-ai-discard" data-act="revert">Reject</button>';
  ...
  _sameBlock(range) {
    ...
    const a = blk(range.startContainer);
    return !!a && a === blk(range.endContainer);
  }
