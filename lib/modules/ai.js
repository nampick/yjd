...
export default class Ai extends Module {
  ...
  _build() {
    ...
    if (this.diffMode) {
      ...
      db.innerHTML =
        '<span class="yjd-ai-diffhint">Click a word to keep/drop it</span>' +
  ...
  _accept() {
    ...
    // Diff-review path: we're editing a non-empty selection and it changed —
    // render a word diff to accept/reject instead of blindly replacing.
