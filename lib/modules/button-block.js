...
export default class ButtonBlock extends Module {
  ...
  constructor(editor, options = {}) {
    ...
    this._onClick = (e) => {
      ...
      // Clicks on the label edit text; anywhere else on the button opens the
      // inspector.
  ...
  openInspector(wrap) {
    ...
    const colorControl = (key) => {
      ...
      if (!theme.strict) {
        ...
        input.addEventListener('change', () => { props[key] = input.value.trim() || props[key]; commit(); });
    ...
    // Position near the button inside the popup container.
    ...
    pop.style.left = Math.max(8, r.left - wrapRect.left) + 'px';
    pop.style.top = (r.bottom - wrapRect.top + 8) + 'px';
  }
