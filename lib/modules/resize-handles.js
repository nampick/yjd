...
class ResizeHandles extends Module {
  ...
  createHandles() {
    ...
    // Live size badge (UI 2.0): "640 × 360" chip pinned to the element's
    // top-right corner while a resize drag is in progress.
    const badge = document.createElement('span');
    badge.className = 'yjd-resize-size';
    badge.style.display = 'none';
    this.sizeBadge = badge;
    this.handlesContainer.appendChild(badge);
  }
  ...
  createAlignBar() {
    ...
    [
      { align: 'left', icon: 'align-left', title: this.t('resize.alignLeft', 'Align left') },
      { align: 'center', icon: 'align-center', title: this.t('resize.alignCenter', 'Align center') },
      { align: 'right', icon: 'align-right', title: this.t('resize.alignRight', 'Align right') }
    ...
    // UI 2.0: divider + destructive delete on the far end of the media bar.
    const sep = document.createElement('span');
    sep.className = 'yjd-image-align-sep';
    bar.appendChild(sep);
    const del = document.createElement('button');
    del.type = 'button';
    del.className = 'yjd-image-align-btn yjd-image-delete-btn';
    del.title = this.t('resize.delete', 'Delete');
    del.setAttribute('aria-label', this.t('resize.delete', 'Delete'));
    del.innerHTML = `<span class="icon">${IconUtils.getIcon('delete')}</span>`;
  ...
  /**
   * Remove the selected image/video (one undoable step) and hide the chrome.
   */
  deleteActiveElement() {
  ...
  handleMouseDown(e, handleName) {
    ...
    // Show the live size badge for the duration of the drag.
    if (this.sizeBadge) {
      this.sizeBadge.textContent = `${Math.round(this.startWidth)} × ${Math.round(this.startHeight)}`;
      this.sizeBadge.style.display = 'block';
    }
  }
  ...
  _applyMouseMove(e) {
    ...
    this.updateHandlePosition();
    if (this.sizeBadge) {
      this.sizeBadge.textContent = `${Math.round(newWidth)} × ${Math.round(newHeight)}`;
    }
  ...
  handleMouseUp(e) {
    ...
    if (this.sizeBadge) this.sizeBadge.style.display = 'none';
  ...
  applyDimensions(width, height) {
    ...
    if (this.activeElement.classList.contains('rich-editor-table')) {
      ...
      this.activeElement.style.width = width + 'px';
      this.activeElement.style.height = height + 'px';
      // A user-driven size wins over the default display caps (e.g. the 360px
      // --rte-video-max-h), so clear them inline — otherwise max-height would
      // clamp an enlarge past the cap.
      this.activeElement.style.maxWidth = 'none';
      this.activeElement.style.maxHeight = 'none';
