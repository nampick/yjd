...
class ResizeHandles extends Module {
  ...
  /**
   * Place the align bar above the image, or BELOW it when there isn't room above
   * (the image sits near the top and the bar would slip under the toolbar).
   */
  positionAlignBar() {
    ...
    const guardTop = toolbar ? toolbar.getBoundingClientRect().bottom
      : this.editor.editor.getBoundingClientRect().top;
    const below = (imgR.top - guardTop) < 46; // 40px bar + gap
    this.alignBar.classList.toggle('below', below);
  }
