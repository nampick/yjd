...
export function calculatePopupPosition(anchor, popup, options = {}) {
  ...
  if (isInWrapper) {
    ...
    if (top + popupHeight > wrapperRect.height && !preferTop) {
      // Try to position above the anchor — but ONLY if it fully fits there.
      const topPosition = anchorRect.top - wrapperRect.top - popupHeight - offsetY;
      if (topPosition >= 0) {
        top = topPosition;
      }
      // Otherwise keep the below-the-anchor position (set above). In a short
      // editor a tall dropdown fits neither below nor above; the popup-container
      // is overflow:visible, so anchoring below lets it extend past the editor
      // and stay attached to its trigger — far better than centering it over the
      // trigger (which hid the control it belongs to).
    }
