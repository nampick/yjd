...
export function calculatePopupPosition(anchor, popup, options = {}) {
  ...
  // Portal detection is STRUCTURAL: since #67 the body portal nests an inner
  // .rich-editor-popup-container (so popup-scoped CSS matches), so the inner
  // container carries the wrapper class while living inside the portal —
  // class-only detection here sent every portaled popover to the viewport's
  // left edge (0×0 container rect + right-overflow clamp).
  const isPortal = !!(container.closest && container.closest('.yjd-popup-portal')) ||
    (container.classList && container.classList.contains('yjd-popup-portal'));
  const isInWrapper = !isPortal && container.classList.contains('rich-editor-popup-container');
