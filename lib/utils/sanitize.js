...
// Inline-style values that can execute script or exfiltrate via CSS: legacy IE
// expression()/behavior/-moz-binding, javascript:/vbscript: URLs, @import, and
// url(javascript:|vbscript:|data:). Matched case-insensitively; control chars
// are stripped before testing so tab/newline smuggling (e.g. "java\tscript:")
// cannot slip a scheme past this check.
const DANGEROUS_STYLE = /expression\s*\(|(?:javascript|vbscript)\s*:|-moz-binding|behavior\s*:|@import|url\s*\(\s*['"]?\s*(?:javascript|vbscript|data)\s*:/i;
 ...
 * @param {boolean} [options.allowDataAV] - allow `data:video/*` and `data:audio/*`
 *   (inert media, no script execution — used for uploaded clips)
 * @param {boolean} [options.allowDataFile] - allow `data:` of inert FILE types
 *   (pdf, zip, plain text, office docs …) for embedded file attachments, while
 *   still blocking script-capable document types (text/html, xhtml, svg, xml)
 * @returns {boolean}
 */
// Data mediatypes that can execute script or be interpreted as a document when
// navigated to — never allowed, even under allowDataFile.
const SCRIPTABLE_DATA_TYPES = new Set([
  'text/html', 'application/xhtml+xml', 'image/svg+xml', 'text/xml', 'application/xml',
]);

export function isSafeUrl(url, { allowDataImage = false, allowDataAV = false, allowDataFile = false } = {}) {
  ...
  if (scheme === 'data:') {
    // Raster image data URIs only. SVG can carry script, so it is denied.
    if (allowDataImage && /^data:image\//i.test(stripped) && !/^data:image\/svg/i.test(stripped)) {
      return true;
    }
    // Audio/video data URIs are inert media (no script execution).
    if (allowDataAV && /^data:(video|audio)\//i.test(stripped)) {
      return true;
    }
    // Embedded file attachments: allow an inert file type, but never a
    // script-capable document type (and never an empty/ambiguous mediatype).
    if (allowDataFile) {
      const m = stripped.match(/^data:([^;,]*)/i);
      const mediatype = (m && m[1] || '').toLowerCase();
      return mediatype !== '' && !SCRIPTABLE_DATA_TYPES.has(mediatype);
    }
    return false;
  }
...
function cleanElement(el) {
  ...
  for (const attr of attrs) {
    ...
    if (name === 'href' || name === 'src' || name === 'xlink:href') {
      ...
      const allowDataAV = el.tagName === 'VIDEO' || el.tagName === 'AUDIO' || el.tagName === 'SOURCE';
      if (!isSafeUrl(value, { allowDataImage, allowDataAV })) {
        el.removeAttribute(attr.name);
      }
      continue;
    }

    // Reject inline styles that can execute script or load remote resources.
    if (name === 'style') {
      const styleValue = value.replace(/[\u0000-\u001F]/g, '');
      if (DANGEROUS_STYLE.test(styleValue)) {
        el.removeAttribute(attr.name);
      }
    }
  }
