...
function videoToMd(node) {
  return `![video](${node.getAttribute('src') || ''})`;
}
...
function inlineMd(s) {
  ...
  return s
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (_, a, src) => mediaMd(a, src))
...
// videos, so yjd output always round-trips back to a player.
function mediaMd(alt, src) {
  ...
  if (/player\.vimeo\.com\/video\//.test(src)) return `<iframe class="inserted-video" src="${attr(src)}" width="560" height="315" frameborder="0" allowfullscreen contenteditable="false"></iframe>`;
  if (/\.(mp4|webm|ogg|mov|avi|mkv)(?:[?#]|$)/i.test(src) || alt.trim().toLowerCase() === 'video')
    return `<video class="inserted-video" src="${attr(src)}" controls style="max-width:100%;height:auto" contenteditable="false"></video>`;
  return `<img class="inserted-image" src="${attr(src)}" alt="${attr(alt)}" style="max-width:100%;height:auto">`;
}
