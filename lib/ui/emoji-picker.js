...
class EmojiPicker {
  ...
  createEmojiPicker() {
    ...
    this.searchInput.placeholder = this._t('emoji.search', 'Search emoji');
    this.searchInput.setAttribute('aria-label', this._t('emoji.search', 'Search emoji'));
    ...
    if (this.categories.length > 1) {
      ...
      const allCats = [{ id: 'recent', icon: '🕘', label: this._t('emoji.recent', 'Frequently used') }, ...this.categories];
      allCats.forEach((c) => {
        const b = document.createElement('button');
        b.type = 'button';
        b.className = 'emoji-cat' + (c.id === this.activeCat ? ' active' : '');
        b.textContent = c.icon;
        const label = this._t('emoji.cat.' + c.id, c.label);
        b.title = label;
  ...
  renderGrid() {
    ...
    let list; // [{ name, emoji }]
    if (this.query) {
      title = this._t('emoji.results', 'Search results');
      list = [];
    ...
    if (!list.length) {
      ...
      empty.textContent = this._t('emoji.none', 'No emoji found');
