...
class ImportPopup {
  ...
  createImportPopup() {
    ...
    title.textContent = this._t('import.title', 'Import File');
    ...
    typeLabel.textContent = this._t('import.fileType', 'File Type');
    ...
    `;
    this.typeSelect.options[0].textContent = this._t('import.selectType', 'Select file type...');
    this.typeSelect.options[1].textContent = this._t('import.html', 'HTML (.html, .htm)');
    this.typeSelect.options[2].textContent = this._t('import.excel', 'Excel/CSV (.csv, .xlsx, .xls)');
    this.typeSelect.options[3].textContent = this._t('import.pdf', 'PDF (.pdf)');
    this.typeSelect.options[4].textContent = this._t('import.word', 'Word (.doc, .docx)');
    ...
    this.dropHint.textContent = this._t('import.hintChooseType', 'Choose a type first, then drop a file here or browse.');
    ...
    this.importButton.textContent = this._t('import.import', 'Import');
  ...
  updateFileInput() {
    ...
    if (selectedType) {
      ...
      this.dropHint.textContent = this._t('import.hintDrop', 'Drop a file here or browse.');
    } else {
      this.fileType = null;
      this.fileInput.disabled = true;
  ...
  _unsupportedReason() {
    if (this.fileType === 'pdf') {
      return this._t('import.needsPdf', 'PDF import needs libraries the core doesn’t bundle.');
    }
    if (this.fileType === 'word') {
      return this._t('import.needsWord', 'Word import needs libraries the core doesn’t bundle.');
    }
    if (this.fileType === 'excel' && this.selectedFile &&
        !/\.csv$/i.test(this.selectedFile.name || '')) {
      return this._t('import.needsExcel', 'Excel files (.xlsx/.xls) need extra libraries — export as CSV instead.');
    }
    return null;
  }
  ...
  async processImport() {
    ...
    try {
      ...
      this.notice.innerHTML =
        '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z"></path><path d="M12 9v4"></path><path d="M12 17h.01"></path></svg>' +
        '<span></span>';
      this.notice.querySelector('span').textContent = this._t('import.error', 'Error importing file: {msg}', { msg: error.message });
    }
  }
