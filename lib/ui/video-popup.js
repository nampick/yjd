...
class VideoPopup {
  ...
  createVideoPopup() {
    ...
    title.textContent = this._t('popup.insertVideo', 'Insert video');
    ...
    textLabel.textContent = this._t('popup.videoUrlLabel', 'Your video url');
    ...
    this.urlInput.placeholder = this._t('popup.videoUrlPlaceholder', 'Please enter your video URL');
    ...
    this.insertButton.textContent = this._t('popup.addVideo', 'Add video');
  ...
  createDropZone() {
    ...
    this.dropZone.innerHTML =
      ...
      `<span class="yjd-dropzone-main">${this._t('popup.dropVideo', 'Drop your video here')}</span>` +
      `<span class="yjd-dropzone-sub">${this._t('popup.dropVideoHint', 'MP4, WebM or MOV · or paste a link above')}</span>`;
  ...
  async insertVideo() {
    ...
    if (!fromUpload) {
      try {
        ...
        if (!isValid) {
          alert(this._t('popup.videoUrlInvalid', 'Invalid video URL. Please check the URL and try again.'));
          return;
        }
      } catch (error) {
        alert(this._t('popup.videoUrlError', 'Error validating video URL.'));
        return;
      }
    }
