...
registerIcons({
  ...
  'list-check': S('<path d="m2.5 6 2 2 3.5-3.5"/><path d="m2.5 17 2 2 3.5-3.5"/><line x1="11" x2="21" y1="7" y2="7"/><line x1="11" x2="21" y1="17" y2="17"/>'),
...
class ListPicker {
  ...
  async createListTypeButtons() {
    ...
    head.className = 'yjd-pick-head';
    head.textContent = 'List type';
    ...
    for (const listType of this.options.listTypes) {
      ...
      // Localisable: 'list.<value>' overrides the English label/tooltip.
      const label = this._t('list.' + listType.value, listType.label);
      listButton.title = label;

      // Design pickers are labelled vertical menus (icon 14px + label).
      const iconSvg = IconUtils.getIcon(listType.icon);
      const short = label.replace(' List', '').replace(' Numerals', '');
      if (iconSvg) {
        listButton.innerHTML = `<span class="icon-wrapper">${iconSvg}</span><span class="label-text">${short}</span>`;
      } else {
        listButton.textContent = short;
      }
  ...
  updateToolbarButtonIcon(listType) {
    ...
    // Update button title (localisable via 'list.<value>')
    button.title = titleMap[listType]
      ? this._t('list.' + listType, titleMap[listType])
      : this._t('list.button', 'List');
  ...
  getListType(listElement) {
    ...
    // Mirror List.getListType: a UL with .checklist is the task list, and the
    // popup's active row must match the toolbar icon (both say "checklist").
    if (listElement.classList && listElement.classList.contains('checklist')) return 'checklist';
    return 'bullet';
  }
