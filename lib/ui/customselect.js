...
class CustomSelect {
  ...
  createSelect() {
    ...
    if (this.options.title) this.popup.setAttribute('aria-label', this.options.title);
  ...
  async createItemList() {
    ...
    this.options.items.forEach(item => {
      ...
      const plainLabel = itemText.textContent || String(this.getItemValue(item) || '');
      if (plainLabel) itemButton.setAttribute('aria-label', plainLabel);
   ...
   */
  getItemDisplay(item) {
    // When an i18nPrefix is set, resolve the plain `title` through the editor's
    // strings ('<prefix>.<value>') and render it as text — localises the format
    // dropdowns (heading/size/case/spacing/line-height) without touching the
    // rich-HTML labels of prefix-less selects (e.g. font-family shows each font
    // in its own face).
    const prefix = this.options.i18nPrefix;
    const ed = this.options.editor;
    if (prefix && ed && typeof ed.t === 'function' && item && item.title != null) {
      const v = item[this.options.valueProperty];
      const s = ed.t(prefix + '.' + v, item.title);
      return `<span>${String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>`;
    }
  ...
  /**
   * Setup Escape-to-close handler so the dropdown is dismissible from the keyboard.
   */
  setupKeyHandler() {
    ...
    this.keyHandler = (e) => {
      if (e.key === 'Escape' && this.isVisible) {
        e.preventDefault();
        this.hide();
        // Return focus to the editor so typing can continue.
        const ed = this.options.editor
  ...
  async show(anchor) {
    ...
    // Store reference to anchor for potential repositioning. If the popup was
    // re-shown against a different trigger without an intervening hide(), the
    // old trigger's pressed state must not stick.
    if (this.currentAnchor && this.currentAnchor !== anchor && this.currentAnchor.classList) {
      this.currentAnchor.classList.remove('open');
    }
    this.currentAnchor = anchor;
    // v2 select spec: the trigger stays visually pressed while open.
    if (anchor && anchor.classList) anchor.classList.add('open');
  }
  ...
  hide() {
    ...
    if (this.currentAnchor && this.currentAnchor.classList) this.currentAnchor.classList.remove('open');
