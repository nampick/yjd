...
registerIcons({
  ...
  'align-justify': S('<path d="M3 6h18"/><path d="M3 12h18"/><path d="M3 18h18"/>')
});
...
class TextAlignPicker {
  ...
  async createAlignmentButtons() {
    ...
    head.textContent = (this.options.editor && typeof this.options.editor.t === 'function')
      ? this.options.editor.t('align.head', 'Text align') : 'Text align';
    ...
    for (const alignment of this.options.alignments) {
      ...
      const label = (e && typeof e.t === 'function') ? e.t('align.' + alignment.value, alignment.label) : alignment.label;
      // Short label: a locale can set 'align.<value>Short' explicitly; else fall
      // back to the English "Align X" → "X" transform (a no-op for other locales).
      const short = (e && typeof e.t === 'function')
        ? e.t('align.' + alignment.value + 'Short', label.replace('Align ', ''))
        : label.replace('Align ', '');
      alignButton.title = label;

      // Design list/align pickers are labelled vertical menus.
      const iconSvg = IconUtils.getIcon(alignment.icon);
      if (iconSvg) {
        alignButton.innerHTML = `<span class="icon-wrapper">${iconSvg}</span><span class="label-text"></span>`;
        alignButton.querySelector('.label-text').textContent = short;
      } else {
        alignButton.textContent = short.charAt(0);
      }
