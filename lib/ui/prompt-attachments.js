...
export default class PromptAttachments {
  ...
  /**
   * Reflect the upload status on the chip (UI 2.0): a pending chip shows an
   * indeterminate progress bar along its bottom edge; a failed one gets the
   * danger ring + a "!" badge. Driven by data-status so integrators that set
   * item.status from 'attachment:add' can call this too (via _sync()).
   */
  _setChipStatus(item) {
    if (!item || !item.el) return;
    item.el.dataset.status = item.status || 'done';
    let bar = item.el.querySelector('.rte-attach-progress');
    if (item.status === 'pending' && !bar) {
      bar = document.createElement('span');
      bar.className = 'rte-attach-progress';
      bar.innerHTML = '<span></span>';
      item.el.appendChild(bar);
    } else if (item.status !== 'pending' && bar) {
      bar.remove();
    }
    let err = item.el.querySelector('.rte-attach-error-badge');
    if (item.status === 'error' && !err) {
      err = document.createElement('span');
      err.className = 'rte-attach-error-badge';
      err.textContent = '!';
      err.title = 'Upload failed';
      item.el.appendChild(err);
