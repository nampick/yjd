...
class ImagePopup {
  ...
  createImagePopup() {
    ...
    title.textContent = this._t('popup.insertImage', 'Insert image');
    ...
    textLabel.textContent = this._t('popup.imageUrlLabel', 'Your image url');
    ...
    this.urlInput.placeholder = this._t('popup.imageUrlPlaceholder', 'Please enter your image URL');
    ...
    cancelButton.textContent = this._t('cancel', 'Cancel');
    ...
    this.insertButton.textContent = this._t('popup.addImage', 'Add image');
  ...
  /**
   * Drag-over drop zone (same recipe as the video popup): while files are over
   * the popup the input group swaps for a dashed target; a dropped image file
   * follows the same path as the upload button.
   */
  createDropZone() {
    ...
    this.dropZone.innerHTML =
      ...
      `<span class="yjd-dropzone-main">${this._t('popup.dropImage', 'Drop your image here')}</span>` +
      `<span class="yjd-dropzone-sub">${this._t('popup.dropImageHint', 'PNG, JPG, GIF or WebP · or paste a link above')}</span>`;
  ...
  _setDropZone(on) {
    if (on && this.selectedImageSrc) return;
  ...
  async handleFileSelect(e) {
    ...
    try {
      ...
      // Keep the original File: insertImage() hands it to editor.insertImageFile
      // so the options.image.upload hook runs; the data URL is preview-only.
  ...
  createPreviewContainer() {
    ...
    this.imagePreview.style.cssText = 'max-width: 100%; max-height: 200px; border-radius: var(--rte-radius-md); object-fit: contain;';
  ...
  async insertImage() {
    // A file upload sets selectedImageSrc (a data: URL of a file whose MIME type
    // was already verified as image/*); a typed/pasted link comes from the input.
    const fromUpload = !!this.selectedImageSrc;
    const src = this.selectedImageSrc || this.urlInput.value.trim();
    const alt = '';

    if (!src) return;

    // Only validate a typed/pasted URL. A file upload already produced a valid
    // image (MIME checked); re-running the URL-format/loadable check on its
    // data: URL is redundant and can wrongly reject it.
    if (!fromUpload) {
      try {
        const { default: Image } = await import('../formats/image.js');
        const isValid = await Image.validateImageUrl(src);
        if (!isValid) {
          alert(this._t('popup.imageUrlInvalid', 'Invalid image URL. Please check the URL and try again.'));
          return;
        }
      } catch (error) {
        alert(this._t('popup.imageUrlError', 'Error validating image URL.'));
        return;
      }
    }
    ...
    // File uploads route through editor.insertImageFile (single insertion path:
    // honours options.image.upload with a placeholder, falls back to a data URL
    // when no hook is set). Only core builds without applyEditorInput — or URL
    // inserts — take the direct path below.
    const editor = this.options.editor;
    if (fromUpload && this.selectedImageFile && editor && typeof editor.insertImageFile === 'function') {
      editor.insertImageFile(this.selectedImageFile);
    } else if (this.options.onImageInsert) {
