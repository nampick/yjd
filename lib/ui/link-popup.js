...
class LinkBar {
  ...
  _build() {
    ...
    const e = this.options.editor;
    const t = (k, fb) => (e && typeof e.t === 'function' ? e.t(k, fb) : fb);
    this.editBtn = mkBtn('link-bar-edit', t('popup.editLink', 'Edit link'),
      S('<path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/>'));
    this.copyBtn = mkBtn('link-bar-copy', t('popup.copyLink', 'Copy link'),
      S('<rect x="9" y="9" width="12" height="12" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h8"/>'));
    this.unlinkBtn = mkBtn('link-bar-unlink', t('popup.removeLink', 'Remove link'),
  ...
  show(linkEl) {
    ...
    this.urlEl.textContent = label || (this.options.editor && typeof this.options.editor.t === 'function'
      ? this.options.editor.t('popup.linkFallback', 'link') : 'link');
